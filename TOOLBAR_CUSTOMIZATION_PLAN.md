...
> **Status:** Planned, not started. Approved design captured here so we can work it
> one session at a time. Delete this file when the whole feature ships (per the
> `RECENTS_PLAN.md` precedent).
...
### Session 7/7 — Show/hide gesture + peek tab

**Goal:** Hide the toolbar with a one-finger double-tap; restore via peek tab or gesture.

**Steps**
1. `GestureDetector` (or manual double-tap timing) in `dispatchTouchEvent`, **finger-only**,
   ignored for stylus/eraser tool types and during an active page/insert swipe. On double-tap,
   toggle `collapsed`, persist, animate-free hide/show.
2. **Peek tab:** a small bordered handle view (`ic_dots` or a thin grab bar) pinned at the
   anchored edge when `collapsed` (for Float, at the last float position). Tap → restore. The
   exclusion rect shrinks to the peek tab while collapsed.
3. `toggleGestureEnabled` setting in the customize dialog (default ON). When the gesture is OFF,
   the peek tab is the only way to collapse/expand — so the tab also acts as a hide button, or
   collapsing is disabled entirely (decide: simplest is the tab toggles regardless).
4. Restore `collapsed` on notebook open from config.

**Test on G10:** Double-tap to hide (finger), peek tab appears at the edge; tap tab or
double-tap to show. Confirm it never fires from the stylus and never collides with page-turn
