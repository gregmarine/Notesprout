...
class ToolbarOverflowManager(
    ...
    /** The FLOAT drag handle (generated by ToolbarLayoutManager), pinned to the leading edge of the
     *  bar. Like [spaceView] it is never moveable and never overflows; it just reserves main-axis
    ...
    /**
     * Set (or clear) the leading-pinned view kept at the start of the bar — the FLOAT drag handle.
     * Forces the next [recalc] to re-derive its state, since the moveable set changed.
     */
    fun setLeadingPinned(view: View?) {
        leadingPinned = view
        initialized = false
    }
    ...
    private fun rebuildAll(inToolbar: List<View>, inOverflow: List<View>, showOverflow: Boolean) {
        ...
        // Clear whatever remains in the toolbar (leadingPinned + dividerOverflow + btnOverflow).
     ...
     *  Padded on the main axis (matching the bar's own end padding) so the leading/trailing button's
     *  white fill never paints over the menu's border stroke. */
    private fun newLine() = LinearLayout(toolbar.context).apply {
        val pad = (4f * toolbar.resources.displayMetrics.density).toInt()
        if (isVertical) {
