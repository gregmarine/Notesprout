 ...
 *
 * Pure Kotlin like the rest of `core/proofread`: the editor feeds it plain text and change
 * offsets, so every decision here is pinned by JVM tests rather than found on a device.
 *
 * The pass is *regional* for the screen's sake, not the checker's: lookups are microseconds, but
 * every span added or removed is e-ink repaint. So an edit re-checks only its own lines — a word
...
object ProofreadCheck {
    ...
    /**
     * Whether inserting or removing [changed] can move flags *outside* its own lines. Backticks
     * and tildes open and close code regions whose reach is the rest of the document — typing the
...
/**
 * Accumulates the span of text edited since the last check, tracked in *current-text* offsets:
 * each new edit first shifts what is already tracked, so a range noted before an earlier
 * insertion still points at the same characters afterwards.
 *
