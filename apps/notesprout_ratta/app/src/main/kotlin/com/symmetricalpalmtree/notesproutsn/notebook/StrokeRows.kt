...
/**
 * The one place a g-paper [Stroke] becomes a `stroke` row and back — part of the family format
 * contract (`docs/data.md` in Paper is the authority): the g-paper stroke id **is** the row id,
 * geometry is format B ([StrokeCodec]) with both pressure and tilt channels written, colour goes
 * through [InkColorCodec], and `style` is the [StrokeStyle] name (unknown name → PEN, so a file
 * written by a future family member with more styles still renders here). Pure Kotlin — JVM-tested.
 */
object StrokeRows {
    ...
    /** Decode one row; null when the blob is missing or malformed — the caller drops the stroke
     *  and the page still renders (a bad row must never take the whole page down with it). */
