...
class ScratchInkTest {
    ...
    fun inwardMintsFreshIdsAndZeroesTime() {
        val out = ScratchInk.toStrokes(listOf(wire(3), wire(2)))
    ...
    fun inwardClampsWidthAndFallsBackToPen() {
        assertEquals(ScratchInk.MAX_WIDTH, ScratchInk.toStrokes(listOf(wire(1, width = 999f)))[0].width, 0f)
        assertEquals(ScratchInk.MIN_WIDTH, ScratchInk.toStrokes(listOf(wire(1, width = 0.01f)))[0].width, 0f)
        assertEquals(StrokeStyle.PEN, ScratchInk.toStrokes(listOf(wire(1, style = "GLITTER")))[0].style)
    }
    ...
    fun outwardDropsIdAndSkipsPointLessStrokes() {
        ...
        val wireStrokes = ScratchInk.toWireStrokes(strokes)
    ...
    fun roundTripKeepsGeometry() {
        val back = ScratchInk.toWireStrokes(ScratchInk.toStrokes(listOf(wire(4))))
