...
class ScratchReadPlanTest {

    private val overhead = ScratchReadPlan.ROW_OVERHEAD.toLong()

    @Test
    fun anEmptyPageIsNoReads() {
        assertEquals(emptyList<LongRange>(), ScratchReadPlan.ranges(emptyList(), emptyList()))
    }

    @Test
    fun onePageThatFitsIsOneRange() {
        val orders = listOf(0L, 1L, 2L, 7L)
        assertEquals(listOf(0L..7L), ScratchReadPlan.ranges(orders, List(4) { 100 }, budget = 10_000))
    }
    ...
    fun packingSplitsAtTheBudget_andCoversEveryOrderExactlyOnce() {
        ...
        val ranges = ScratchReadPlan.ranges(orders, lengths, budget)
    ...
    fun aLoneOversizeStrokeGetsItsOwnRange() {
        ...
        val ranges = ScratchReadPlan.ranges(orders, lengths, budget = 1_000)
    ...
    fun nonContiguousOrdersStillSelectExactlyTheirStrokes() {
        ...
        val ranges = ScratchReadPlan.ranges(orders, List(4) { 500 }, budget = (2 * (500 + overhead)).toInt())
    ...
    fun ordersAndLengthsMustAgree() {
        try {
            ScratchReadPlan.ranges(listOf(0L), emptyList())
