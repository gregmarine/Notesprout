 ...
 *   which text documents do not get an exception from). It appears in the **notebook scope only**,
 *   og's width rule: a page's document needs the room the `‹ n / m ›` cluster takes, and in the
 *   notebook scope that cluster is GONE, so the width is already there. Hiding rather than disabling
 *   is this app's rule everywhere — a disabled control is visually silent on e-ink.
...
object TextDocumentRules {
     ...
     * @param scope the adopted target's scope — the button rides the notebook scope only.
     */
    fun showsPages(textDocument: Boolean, scope: Int): Boolean =
        textDocument && scope == DocumentContract.SCOPE_NOTEBOOK
