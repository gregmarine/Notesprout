 ...
 * and — when the correction is mechanical — a one-tap [replacement] for the flagged range. [rule]
 * names the rule that fired, stable across passes; session-scoped ignores key on it.
 */
 ...
 * There is no viable offline grammar library for this app, and a noisy home-grown one would be
 * worse than none — every rule here prefers silence to a wrong flag, and every guard below exists
 * because the unguarded rule fires on legitimate prose.
 *
 * Pure Kotlin like the rest of this package; Markdown awareness comes from the same
 * [ProofreadTokenizer] mask the spelling pass uses, so code, URLs, and link targets are never
 * judged. Every rule reads only within one line — a document's paragraphs are single
 * (soft-wrapped) lines after reflow — which keeps the editor's line-bounded incremental re-check
 * exact for grammar just as it is for spelling.
 */
object GrammarRules {
    ...
    /**
     * As [check], against an already-computed mask and span list — the editor's pass tokenizes once
     * and hands the same mask to the spelling check and here.
     */
    fun check(
        text: String,
    ...
    /**
     * "i" the pronoun, and its contractions (i'm, i'll, i've, i'd). Guards for the bare letter: an
     * adjacent hyphen (i-beam) or a following period (i.e.) means it is notation, not a word.
     */
     ...
     * takes "an". Acronyms, single letters, digits, and cross-line pairs are never judged. The flag
     * sits on the article alone, so the fix touches one word.
     */
    ...
    /**
     * Per line (multi-line pairs are legitimate in prose; a line is the safe judging unit — and the
     * incremental region's, too): straight double quotes must pair up, smart quotes must balance,
     * and `()[]{}` must nest. Guards: masked characters (code, URLs, link targets) are invisible
     * here; a quote after a digit is inches/seconds; a lone `)` after an emoticon eye or a short
     * enumeration marker ("1)", "a)") is deliberate. No fix — which side is wrong is the writer's
     * call.
     */
