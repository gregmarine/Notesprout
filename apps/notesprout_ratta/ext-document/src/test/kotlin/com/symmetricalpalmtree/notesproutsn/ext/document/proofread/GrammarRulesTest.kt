...
class GrammarRulesTest {
    ...
    @Test
    fun punctuation_between_the_pair_breaks_the_repeat() {
        // Only same-line whitespace joins the two words; anything else is a clause boundary that
        // happens to repeat a word.
        none("birds, birds everywhere", GrammarRules.RULE_REPEATED)
        none("he said stop. stop again", GrammarRules.RULE_REPEATED)
    }
    ...
    @Test
    fun a_word_with_a_digit_never_opens_a_sentence_flag() {
        // The rule judges all-letter words only — a token carrying a digit has no "capital" form
        // worth suggesting.
        none("It ended. 2nd try worked", GrammarRules.RULE_CAPITALIZE)
        none("It ended. e2e tests ran", GrammarRules.RULE_CAPITALIZE)
    }
    ...
    @Test
    fun a_hyphen_on_either_side_means_notation() {
        none("the plot of x-i values", GrammarRules.RULE_LONE_I)
    }
    ...
    @Test
    fun an_article_inside_code_is_invisible() {
        none("run `a apple` now", GrammarRules.RULE_A_AN)
    }
    ...
    @Test
    fun an_empty_region_yields_nothing() {
        val text = "the the birds"
        assertTrue(GrammarRules.check(text, ProofreadCheck.Region(4, 4)).isEmpty())
    }

    @Test
    fun the_shared_mask_overload_agrees_with_the_convenience_one() {
        // The editor's pass tokenizes once and hands the same mask here; both entry points must
        // find the same flags.
        val text = "a apple and the the rest (unclosed\ncode `if (x` stays quiet"
        val region = ProofreadCheck.Region(0, text.length)
        val skip = ProofreadTokenizer.skipMask(text)
        assertEquals(
            GrammarRules.check(text, region),
            GrammarRules.check(text, region, skip, ProofreadTokenizer.wordSpans(text, skip)),
        )
    }
