...
/**
 * One chunk of an ink transfer (arc 11 / J3): [strokes] plus the page px geometry they were authored
 * in ([pageWidth] × [pageHeight]; `0f × 0f` = unknown → the reader uses its own page). The host
 * chunks a transfer at [ExtensionContract.TRANSFER_CHUNK_STROKES] /
 * [ExtensionContract.TRANSFER_CHUNK_POINTS] per Binder call ([InkChunks]); the reader re-checks
 * ([requireValid] in the constructor → at unmarshal). An empty bundle (0 strokes) is legal — it is
 * how `takeOutgoing` says "done".
 *
...
class InkBundle(
    ...
    private constructor(parcel: Parcel) : this(
        ...
        strokes = parcel.createTypedArrayList(WireStroke.CREATOR)?.filterNotNull() ?: emptyList(),
    )
