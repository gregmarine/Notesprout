...
/** Copied from NSE · Sketch (arc 51): the grid's geometry: square cells, one count across the width, symmetric about
...
class GridLayoutTest {
    ...
    private fun plan(count: Int, kind: Int = Grid.LINES) =
    ...
    @Test fun offIsNothing() {
        assertNull(GridLayout.plan(Grid.OFF, 4, w, h))
        assertNull(GridLayout.plan(Grid.LINES, 0, w, h))
        assertNull(GridLayout.plan(Grid.LINES, 4, 0, h))
    }

    @Test fun cellIsWidthOverCount() {
        for (count in Grid.COUNTS) assertEquals(w.toFloat() / count, plan(count).cell, 1e-4f)
    }
    ...
    @Test fun dotsAreEveryCrossing() {
        val lines = plan(6)
        val dots = plan(6, Grid.DOTS)
        assertEquals(Grid.DOTS, dots.kind)
    ...
    @Test fun landscapePageIsSymmetricToo() {
        val p = GridLayout.plan(Grid.LINES, 4, h, w)!!
