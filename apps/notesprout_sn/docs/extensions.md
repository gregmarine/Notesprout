...
| `:ext-sketch` | Android application (its own installable APK) | `:extension-api` + `:sn-screen` + `:ext-ink`; **never** `:app` | the SIXTEENTH module (arc 43 "Sketch" / K5, grown by K5b, **NSE · Sketch**, the family's puzzle icon, the second extension living outside `apps/notesprout_sn` — `extensions/sketch/`, included by `projectDir`; manifest declares API **18** since K5b): `SketchApplication` (`RattaEngine.register()`), `SketchService` (the `ISketch` stub — `begin(host)`/`end()`, no store; park re-push in `end()` on a host restart), `SketchSession`, `SketchActivity : PaperScreenActivity` (`pageMode = RASTER`, pencil + rubbing eraser only, no lasso, no smart lasso, no scribble erase), `SketchSaver`/`SketchSaveGovernor`/`PendingPngPark` (pen-idle-debounced PNG push, the parked-save recovery), `RasterTiles`/`RasterEditBuilder`/`SketchEdit` (the Paintsprout Onyx raster-undo port, 64 px tiles, a 48 MB budget), `RasterImage` (PNG encode/decode behind `PngHeader`), `InkBake` ("Bring in ink"), `PageTurn` (pure page-turn math, `reindexAfterInsert`/`reindexAfterDelete` since K5b) — see `extensions/sketch/docs/sketch.md` |
...
`SketchHostBinder`'s uid check, revoked with the unbind. Six methods at API 17 — `current()`,
`requestPage(direction)`, `readSketchChunk(i)`, `saveSketchChunk(pageKey, i, bytes, last)`,
`requestInk(pageKey): chunkCount`, `readInkChunk(i)` — and five more at API 18 (K5b, § below).
PNG crosses **chunked both ways** at `SKETCH_CHUNK_BYTES` = 512 KiB (`ByteChunks`, the
`TextChunks` recipe ported to bytes); `requestInk` answers **0** for a page with no bare ink,
never a refusal — the pad's Send refuses because a selection can be made smaller, but a face has
no selection to shrink, so a dense page arrives **cut** at the transfer caps instead. A save is
accepted for **any live page** of the open notebook, identity by key, no mode routing; a key
naming a deleted page is `IllegalArgumentException("Unknown page")`. Nothing rides the screen's
Intent but `EXTRA_CHROME_HIDDEN` — the editor's and the cloud connect screen's precedent again.
`RESULT_SKETCH_SHOW_PAGES` is a plain result code, not a stub method; Back is `RESULT_CANCELED`.

### The one written-down hard refusal

`SketchContract.MAX_BYTES` = 6 MiB is a **hard refusal** — the SQLCipher cursor window means a PNG
written above it can never be read back, so `SketchRepository.save` throws `SKETCH_TOO_LARGE`
before a byte is written, verbatim through the seam. Every other row type in this family holds
"never refuse" as a design law; this is the one deliberate, written-down exception. `WATCH_BYTES`
= 4 MB is log-only, well below it.
...
| 65 | **Arc 43: `ISketchHost`'s six K2 methods move a page's PNG and its bare-ink strokes across, chunked both ways, and nothing else.** `current()`/`requestPage(direction)` answer a `SketchPageState` (key, index, count, width, height, byte count, chunk count — validated in its own `init`); `readSketchChunk`/`saveSketchChunk` move the PNG at `SKETCH_CHUNK_BYTES` (512 KiB) with the sender's key checked against the receiver's window on every chunk; `requestInk(pageKey)` answers a chunk count with **0 meaning no bare ink, never a refusal** (link-wrapped and sticky ink are excluded structurally, not by a filter here) and `readInkChunk` streams the same `WireStroke` shape the pad's transfer already uses. `SketchHostBinder.gate()` runs first in all six; only the six's own request/reply parcelables ever leave the `hook {}` funnel. | `ISketchHost.aidl`, `SketchPageState` (constructor checks), `ByteChunks`, `SketchHostBinder`, `SketchHostSession`, `SketchHostHooks.loadCurrent/requestPage/commit/requestInk` |
| 66 | **Arc 43: `MAX_BYTES` is the one written-down hard refusal in the whole family, and it is enforced host-side before a byte crosses.** `SketchContract.MAX_BYTES` = 6 MiB is the SQLCipher cursor-window ceiling — a PNG stored above it could never be read back — so `SketchRepository.save` throws `SKETCH_TOO_LARGE` (the contract's exact string) before any `saveSketchChunk` write lands, and a refused **stored** row (one that shrinks after already being saved smaller, then grows past the line on a later save) is soft-deleted rather than left half-written. `SKETCH_BAD_PNG` is the IHDR guard's twin refusal, thrown before decode on either side of the seam. | `SketchContract.MAX_BYTES`/`WATCH_BYTES`/`SKETCH_TOO_LARGE`/`SKETCH_BAD_PNG`, `SketchRepository.save`, `PngHeader.matches` |
