...
| API version | **per service**: the editor declares **14** since arc 39 "Lookup" (2026-09-13 — `DocumentContract.MIN_API_VERSION_FOR_DOCUMENT_LOOKUP`: it calls `IDocumentHost.openReference`, transaction code 12, which a 13 host would land on nothing; **6** from arc 22 / X4 to arc 38 — `MIN_API_VERSION_FOR_STORE`; the Document button was gone from the host between X1's landing and X4's, when this was still 2), the text importer and document exporter stay at **3** (the `resultKind` tail / `SOURCE_DOCUMENT` are load-bearing for them, and neither takes a store — an older host skips those two services at discovery and still binds nothing it would misread) |
