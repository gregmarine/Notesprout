...
| `:ext-sketch` | Android application (its own installable APK) | `:extension-api` + `:sn-screen` + `:ext-ink`; **never** `:app` | the SIXTEENTH module (arc 43 "Sketch" / K5, grown by K5b, **NSE · Sketch**, the family's puzzle icon, the second extension living outside `apps/notesprout_sn` — `extensions/sketch/`, included by `projectDir`; manifest declares API **18** since K5b): `SketchApplication` (`RattaEngine.register()`), `SketchService` (the `ISketch` stub — `begin(host)`/`end()`, no store; park re-push in `end()` on a host restart), `SketchSession`, `SketchActivity : PaperScreenActivity` (`pageMode = RASTER`, pencil + rubbing eraser only, no lasso, no smart lasso, no scribble erase), `SketchSaver`/`SketchSaveGovernor`/`PendingPngPark` (pen-idle-debounced PNG push, the parked-save recovery), `RasterTiles`/`RasterEditBuilder`/`SketchEdit` (the Paintsprout Onyx raster-undo port, 64 px tiles, a 48 MB budget), `RasterImage` (PNG encode/decode behind `ImageHeader`), `InkBake` ("Bring in ink"), `PageTurn` (pure page-turn math, `reindexAfterInsert`/`reindexAfterDelete` since K5b) — see `extensions/sketch/docs/sketch.md` |
