...
*eighth* capability point without another user decision** (`apps/notesprout_sn/CLAUDE.md`).
...
by **`NSE · Cloud Storage`** (`:ext-cloud`), the **thirteenth** module and the **only** one carrying
...
**Arc 43 "Sketch" is the ninth fresh user decision, on 2026-09-15.** The user asked for a raster
sketch beside each page's ink, so SN gains a **tenth** capability point, `ACTION_SKETCH` +
`ACTION_SKETCH_SCREEN` — the sixth screen-owning point, and the second **with paper** after the
scratch pad and calendar (the pad's shape: an extension writes nothing to disk itself, the host
owns every `.soil` write) — served by **`NSE · Sketch`** (`:ext-sketch`), the **sixteenth** module
and the **second** living outside `apps/notesprout_sn` (`extensions/sketch/`, the Bible
`projectDir` pattern repeated). `API_VERSION` 16 → 17, a compatible addition floored at 17
(`SketchContract.MIN_API_VERSION_FOR_SKETCH`, a birth floor — no existing floor moves, only
`:ext-sketch` declares 17); grown the same arc (K5b) by five more `ISketchHost` tails
(`insertPage`/`deletePage`/`pageContent`/`undoPage`/`redoPage`) under a **method** floor,
`API_VERSION` 17 → 18, `MIN_API_VERSION_FOR_SKETCH_PAGES` = 18 — `MIN_API_VERSION_FOR_SKETCH` stays
17 untouched. Plan `extensions/sketch/SKETCH_PLAN.md`; reference doc once frozen
`extensions/sketch/docs/sketch.md`. The rule survives once more, another word wider: **no
*eleventh* capability point without another user decision** (`apps/notesprout_sn/CLAUDE.md`).
...
`extensions/bible/docs/bible.md`; the sketch face likewise — `extensions/sketch/docs/sketch.md`.
This doc is the seam for all ten points, and the store rebuild that underlies three of them.
...
reference — nothing is copied, and SN's AIDL is scoped to its **ten** points rather than Paper's
broader capability set. Paper never built export, import, documents, tags, a calendar, a cloud
destination, a Bible reader or a raster sketch, so it has nothing to say about the third through
tenth; og's
...
nothing of its app shell; Paintsprout Onyx's raster sketchbook (`apps/paintsprout_onyx`, a sister
project) the tenth's: `RasterTiles`/`RasterRows`/`RasterImage`/`UndoRedoStack` were ported, host-side
and device-neutral, not the Onyx app shell — the raster **engine** work (pencil, rubbing eraser,
Ratta cadence) landed in `~/git/g-paper` first. Tags have no og reading reference — the feature is
new to this family.
...
Sixteen modules (fourteen in this folder and two at the monorepo root), SN's own Gradle root:
...
| `:ext-bible` | Android application (its own installable APK) | `:extension-api` + `:sn-screen`; **never** `:app`, no Room / SQLCipher / serialization | the FIFTEENTH module (arc 37 / B0–B3, grown by arc 38 / R1–R2 and arc 40 / V1, **NSE · Bible**, the family's puzzle icon, the first extension living outside `apps/notesprout_sn` — `extensions/bible/`, included by `projectDir`; manifest declares API **15** since arc 40 "Verses"): `BibleService` (the `IBible` stub — `begin(store)`/`end()`/`resolve(text)`/`beginAt(store, reference)`/`takeOutgoingReference()` (B9)/`passageText(wire)` (arc 40), `HostCallerCheck.enforce` first), `BibleSession` (grew a `reference` field at R1 and an `outgoing` reference at B9), `Reference.kt` (`Passage`, `ReferenceParser`, `ReferenceCodec`, `ReferenceResolver` — arc 38 / R1), `PassageLoader`/`PassageAtoms` (arc 38 / R2), `PassageMarkdown` (arc 40 / V1 — the verses as Markdown, and the ten-verse/no-whole-chapter cap `passageText` enforces before reading a row), `BibleSchema` (schema v3 since R2: `state(key, value)` — the calendar's `state` / the editor's `prefs` precedent — plus `recent(usfm, chapter, at)` keyed on the chapter and `recent_ref(ref, at)` keyed on the wire; `INSERT OR REPLACE` safe on all three, no children), `BibleSql`, `BibleStore`, `ContentInstaller` (the bundled `assets/bible/bsb.bible` → `noBackupFilesDir/bible/`, stamped `lastUpdateTime:size` — **the one sanctioned "extension writes to disk" exception**: re-derivable APK content, never user data, ML Kit's own model's class), `BibleDatabase`, `VerseKey`/`Canon`, `ChapterCursor`, `Position`, `ChapterLoader`, `reader/` (`Atom`, `ChapterPaginator` with its `BodyMeasurer` seam, `ReaderTypography`, `ReaderView`), `BibleActivity` (the Send chooser's "reference or verses" since arc 40), `ContentsModel`/`ContentsLayout`/`ContentsPanel` (B6, named by B7), `RecentChapters`/`RecentsPanel` (B7) — see `extensions/bible/docs/bible.md` |
| `:ext-sketch` | Android application (its own installable APK) | `:extension-api` + `:sn-screen` + `:ext-ink`; **never** `:app` | the SIXTEENTH module (arc 43 "Sketch" / K5, grown by K5b, by arc 44 "Pencils" and by arc 45 "Ink" / G3, **NSE · Sketch**, the family's puzzle icon, the second extension living outside `apps/notesprout_sn` — `extensions/sketch/`, included by `projectDir`; manifest declares API **20** since arc 45 / G3, moved from its birth 17 — 18 at K5b, 19 at arc 44): `SketchApplication` (`RattaEngine.register()`), `SketchService` (the `ISketch` stub — `begin(host)`/`end()`, no store; park re-push in `end()` on a host restart), `SketchSession`, `SketchActivity : PaperScreenActivity` (`pageMode = RASTER`, pencil + rubbing eraser (graphite only, ink never erased) + a 5 px gel pen (ink), no lasso, no smart lasso, no scribble erase), `SketchSaver`/`SketchSaveGovernor`/`PendingImagePark` (renamed from `PendingPngPark` at arc 45 / G3; pen-idle-debounced WebP push per raster, the parked-save recovery), `RasterTiles`/`RasterEditBuilder`/`SketchEdit` (the Paintsprout Onyx raster-undo port, 64 px tiles, a 48 MB budget, each entry carrying a raster layer since arc 45), `RasterImage` (lossless WebP encode/decode behind `ImageHeader`, `WEBP_EFFORT` = 100), `SketchLayers` (the `SketchContract.LAYER_*` ↔ `RasterLayer` translation, arc 45 / G3), `InkBake` ("Bring in ink", lands in the ink raster since arc 45), `PageTurn` (pure page-turn math, `reindexAfterInsert`/`reindexAfterDelete` since K5b) — see `extensions/sketch/docs/sketch.md` |
| `:app` (`extension/` package) | part of the host APK | `:extension-api` | `ExtensionRegistry`, `ExtensionBinder`, `ExtensionCallException`, `InkCaps`, `RecognizerClient`, `RecognizerReadiness`, `HeldInkClient` (arc 23 / Y4 — the pad's and the calendar's held-bind lifecycle written once: `HeldInkPoint` is the per-point names/budgets interface, `DrainedInk` the one drained-result class), `ExtensionScreenEntry` (Y4 — the pad's and the calendar's entry-button door written once: `InkSend` the one outbound-ink class, `EntryWording` the four strings), `TransferSelection` (Y4 — the pure ink-only/writing-order rule both lasso sends obey), `ScratchPadClient` / `CalendarClient` / `ScratchPadEntry` / `CalendarEntry` (since Y4, thin points on the two classes above — a point's companion is a `HeldInkPoint`, its constructor a set of `ExtensionScreenEntry` wiring), `TransferCaps`, `ExporterClient`, `ImporterClient`, `DocumentEditorClient`, `DocumentEditorEntry`, `DocumentHostBinder`, `DocumentHostSession`, `TagClient`, `TagManagerEntry`, and (arc 25) `CloudClient` (the store-taking, bind-per-call file ops — no held bind), `CloudConnectClient` (the one held bind on the point, the connect showing's bracket), `CloudConnectEntry` (the entry-button door over it — registered in `onCreate`, closed in `onDestroy`, always answers even on the sign-in-could-not-open path), `CloudArgs` (host-side checks run before any bind), `CloudTimeouts` (the measured per-method budget table), `CloudWording`, `CloudNotConnectedException` / `CloudNetworkException`; and in `data/extstore/`, the extension store — rebuilt on `SupportSQLiteOpenHelper` at arc 22 / X1, Room's `KvEntity`/`KvDao` deleted with it (`ExtensionStores`, `ExtensionStoreDatabase`, `StoreFormat`, `StoreExecutor` / `SupportStoreExecutor`, `ExtensionStoreGate`, `ExtensionStoreBinder`) — plus, in `export/` and `crypto/`, export's own host-side half (`ExportActivity`, `ExportPanel`, `ExportOptions`, `ExportArtifact`, `ExportNaming`, `ExportKeying`, `SoilOpenFiles`, and arc 19's `ExportText`, `ExportDocumentRules`, `DocumentPdfRender`, `DocumentPdfMetrics`), in `importing/` and `crypto/`, import's (`ImportFlow`, `NotebookImport`, `ImporterMatch`, `ImportNames`, `AncestryPlan`, `SafeImportId`, `ImportDialogs`, `ImportOverlay`, `ImportKeying`, `NotebookRemap` in `data/soil/`, and arc 19's `TextImport`), in `notebook/`, tags' own host-side half (`TagsPopup`, `TagTargets`, `TagSelection`), and in `notebook/`, the calendar's own host-side half (`CalendarTargets`, arc 23 / Y3 — the four Send-to-Calendar choices, every one through `CalendarTarget.of`); and, arc 25, the cloud point's three consumers' own host-side halves — in `export/`, `ExportDestination` (the pure Destination-row rules) and `cloud/CloudBrowserDialog` + `CloudBrowserRules` (the host-drawn folder/file browser, shared with import); in `data/backup/`, `DeviceFolder`, `SelfContainedSnapshot` (the WAL-absorbing cache copy — the cloud never holds a sidecar) and `CloudBackupLeg` + `CloudBackupRules` (the backup run's second leg); in `importing/`, `CloudImportRules` (the download's three-way byte corroboration); and, arc 37, the Bible point's own host-side half — `BibleClient` (`TagClient`'s shape minus `configureShowing` — the Bible interface has nothing to tell the reader; grew the bind-per-call `Companion.resolve` and an `open(reference)` overload at arc 38 / R1) and `BibleEntry` (`TagManagerEntry`'s skeleton + `ExtensionScreenEntry`'s surface-stack push/pop, since unlike Tags the Bible IS a restore surface — `Surface.BIBLE`; grew `supportsReferences`/`resolve`/`open(reference)` at arc 38 / R1, and in `notebook/`, arc 38 / R3's own host-side half — `BibleRefFlow`, `BibleRefDialog`) |
...
|---|---|---|
| `API_VERSION` | **20** (arc 45 "Ink" / G2 2026-09-17; 19 = arc 44 / T2 "Pencils" 2026-09-17; 18 = K5b "Sketch pages" 2026-09-15; 17 = arc 43 / K2, the Sketch point; 16 = arc 42 "Notes"; 15 = arc 40 "Verses"; 14 = arc 39 "Lookup"; 13 = B9 "Send"; 12 = arc 38 / R1; 11 = arc 37 / B0, 10 = arc 35 / HA1, 9 = arc 31 / HV1, 8 = arc 25 / V1, 7 = arc 23 / Y1) | the host accepts a service whose `<meta-data>` is in `minApiVersion(action)..API_VERSION` (`ExtensionContract.accepts`) — the declared number is what the extension *requires* of the host. `minApiVersion` answers **`MIN_API_VERSION_FOR_STORE`** (6) for the three arc-22 **store-taking** points (scratch pad, document editor, tag manager), **`MIN_API_VERSION_FOR_CALENDAR`** (7) for the calendar, **`MIN_API_VERSION_FOR_BIBLE`** (11) for the Bible point's own existence, **`SketchContract.MIN_API_VERSION_FOR_SKETCH`** (**20**, moved from its birth 17 at arc 45 / G2) for the Sketch point's own existence, and 1 for every stateless point — a service below its floor is skipped even though the ceiling alone would admit it. Meta-data is **per service**: the PDF exporter declares 2 (the `sourceKind` tail), `:ext-document`'s text importer and document exporter declare 3 (the `resultKind` tail / `SOURCE_DOCUMENT`), its editor service declares **6** (arc 22 / X4, since it takes a store), `:ext-tags`' one service declares **6** (arc 22 / X3), `:ext-scratchpad`'s one service declares **6** (arc 22 / X2), `:ext-calendar`'s one service declares **10** since arc 35 / HA1 (`advanceOutgoing`; 9 from arc 31 / HV4 for the two appended `ICalendar` methods; 7 from arc 23 / Y1 to HV3 — the point was born at 7, so there is no lower number to consider), everything else (`:ext-mlkit`, `:ext-soil`) at 1. **The ledger:** 2 = arc 18's `sourceKind` tail · 3 = arc 19 / M8's `resultKind` tail · 4 = arc 21 / W1, the tag point itself · 5 = arc 21 / W4, the first bump that is not a compatible tail · 6 = arc 22 / X1, the second break and the first that carries a floor · **7 = arc 23 / Y1, the calendar point — a compatible *addition* (no existing interface changes shape) and the first bump with a *per-action* floor** — see the version note below. **Arc 24 "Events" (2026-09-02) did NOT bump it** — the first arc since 21 to leave the ledger alone: it grew the calendar's own screen in place, touched no existing interface and opened no new point, so `:ext-calendar`'s service keeps declaring the same 7 it declared at Y1. **Arc 28 "Objects" (2026-09-06) did NOT bump it either** — the `bundleVersion` tail added at H6 is compatible in exactly the shape `sourceKind` already was, so `:ext-pdf` keeps declaring whatever it already declared. **8 = arc 25 / V1, the cloud point** — the calendar's shape, floored at 8 (`CloudContract.MIN_API_VERSION_FOR_CLOUD`), no existing door moved. **9 = arc 31 / HV1, `ExporterInfo`'s third compatible tail `delivery`** (`DELIVERY_ONE_FILE` / `DELIVERY_PER_PAGE`, absent = one file) — and, at HV4 under the same number, two methods appended to `ICalendar` (`render`, `outgoingTarget`). **Not a ninth point and no floor moves**: `MIN_API_VERSIONS` is untouched, every existing extension keeps its declaration. The bump is the arc-18 skew guard, not a parcel need: a per-page exporter facing an API-8 host would be handed a whole-notebook bundle and asked for one file, so `:ext-image` declares **9** (`ExporterContract.MIN_API_VERSION_FOR_DELIVERY`) and an older host skips it at discovery; the host itself reads the `delivery` tail only from a service declaring ≥ 9 (`ExportDelivery.delivery`), so the declaration and the tail can never disagree about what the host will do. The calendar's `render` is a *method* floor (offered only to a calendar declaring 9), never an action floor — a 7 still binds for everything it always did. **10 = arc 35 / HA1, `ICalendar.advanceOutgoing`** — a Day's whole-page send parks both halves and the host drains them in turn; appended after `outgoingTarget`, method floor `MIN_API_VERSION_FOR_CALENDAR_DAY_SEND` = 10, no action floor moved, no other extension redeclares. **11 = arc 37 / B0, the Bible point** — the calendar's/cloud's shape: a compatible addition floored at 11 (`MIN_API_VERSION_FOR_BIBLE`), no existing floor moved, only `:ext-bible` declares 11. **12 = arc 38 / R1, `IBible.resolve` + `IBible.beginAt`** — two methods appended after `end()`, the calendar's `render`/`advanceOutgoing` shape once more: a **method** floor, `MIN_API_VERSION_FOR_BIBLE_REFERENCE` = 12, gates only the two new methods and the notebook's reference doors; `MIN_API_VERSION_FOR_BIBLE` stays 11 untouched, so an 11-declaring reader keeps every door it already had. Not a tenth point and no action floor moves; only `:ext-bible` declares 12. **13 = B9 "Send" (2026-09-13), `IBible.takeOutgoingReference`** — one method appended after `beginAt`, method floor `MIN_API_VERSION_FOR_BIBLE_SEND` = 13, gates only the reader's Send to notebook (the host sets `EXTRA_BIBLE_SEND_ENABLED` only against a 13); nothing else moves, only `:ext-bible` declares 13. **14 = arc 39 "Lookup" (2026-09-13), `IDocumentHost.openReference`** — one method appended after `closeNotebook` on the **host-side** stub (the first bump for one): the editor's selection is resolved by the host and parked, and the editor then starts the host's exported `BibleLookupActivity` (`DocumentContract.ACTION_DOCUMENT_LOOKUP_SCREEN`) which opens the reader over the editor. The floor is on the *extension's* declaration, `DocumentContract.MIN_API_VERSION_FOR_DOCUMENT_LOOKUP` = 14: `:ext-document`'s editor service redeclares 14 (an editor that calls code 11 must never bind a 13 host — the arc-18 skew guard), and the host puts `EXTRA_DOCUMENT_BIBLE_AVAILABLE` on the editor's Intent only against a 14. `MIN_API_VERSION_FOR_STORE` stays 6, no action floor moves; `:ext-bible` declares 15 as of the next entry below, not 13. **15 = arc 40 "Verses" (2026-09-13), `IBible.passageText`** — one method appended after `takeOutgoingReference` (code 6), the calendar's `render`/arc-38's `resolve`/`beginAt` shape once more: a compatible addition, method floor `MIN_API_VERSION_FOR_BIBLE_TEXT` = 15, `MIN_API_VERSION_FOR_BIBLE` still 11 untouched. `passageText(wire)` answers `PassageText(status, text)` (`STATUS_OK`/`STATUS_TOO_LONG`, `MAX_TEXT_CHARS` 4000) — the first time scripture crosses the seam in either direction, gating only the reader's Send-as-verses and the notebook's two verses doors (the reference doors from arc 38 are untouched and still gated at 12). Not a tenth point; only `:ext-bible` redeclares. **16 = arc 42 "Notes" (2026-09-14), seven `IBible` tails** — six store-taking, bind-per-call pushes (`replacePageNotes`/`replaceNotebookNotes`/`renameNotebookNotes`/`deleteNotebookNotes`/`pruneNotes`/`noteDocumentReference`, transaction codes 7–12, the tag manager's `assign` shape) plus one held-bind read (`takeOutgoingNote`, code 13, `takeOutgoingReference`'s twin), all behind the method floor `MIN_API_VERSION_FOR_BIBLE_NOTES` = 16; `MIN_API_VERSION_FOR_BIBLE` still 11 untouched. The host keeps pushing where a Bible reference sits (notebook, page, kind, wire, stamp) into the reader's own `note_ref` index and puts `EXTRA_BIBLE_NOTES_ENABLED` on the showing's Intent only against a 16-declaring reader with a notebook page behind it. Not a tenth point; only `:ext-bible` redeclares. **17 = arc 43 / K2 (2026-09-15), the SKETCH point** — the calendar's/Bible's shape: a compatible addition floored at 17 (`SketchContract.MIN_API_VERSION_FOR_SKETCH`, a birth floor), no existing floor moved, only `:ext-sketch` declares 17. This is the **tenth** point, not an eleventh. **18 = arc 43 / K5b (2026-09-15), five `ISketchHost` tails** — `insertPage`/`deletePage`/`pageContent`/`undoPage`/`redoPage` appended after `readInkChunk`, behind the **method** floor `MIN_API_VERSION_FOR_SKETCH_PAGES` = 18; `MIN_API_VERSION_FOR_SKETCH` stays 17 untouched, so a 17-declaring sketch extension keeps the plain face without page insert/delete/undo/redo. Only `:ext-sketch` redeclares. **19 = arc 44 / T2 (2026-09-17), two `ISketchHost` tails** — `toolSettings()` / `putToolSettings(in SketchToolSettings)` appended after `redoPage` (codes 12–13) on the host-side stub, behind the **method** floor `MIN_API_VERSION_FOR_SKETCH_TOOLS` = 19; `MIN_API_VERSION_FOR_SKETCH` stays 17, `MIN_API_VERSION_FOR_SKETCH_PAGES` 18 untouched; only `:ext-sketch` redeclares. **20 = arc 45 "Ink" / G2 (2026-09-17) — the THIRD non-tail break (after arc 21 / W4's `TagShowing` and arc 22 / X1's store), and the first to move a single point's ACTION floor after its birth (X1 moved three at once, to 6, when floors were introduced).** A sketch is two rasters now (graphite + ink, `INK_PLAN.md`), so `ISketchHost.readSketchChunk` / `saveSketchChunk` (transaction codes 3–4) take a raster layer **in place**, `SketchPageState`'s wire form grows a second byte/chunk pair, and the image guard becomes WebP (`ImageHeader`, `PngHeader` gone) — neither side can read the other's parcel, so this is not a tail a floor can sit below. `SketchContract.MIN_API_VERSION_FOR_SKETCH` moves **17 → 20**, granted by the user's decision 4 (no legacy, no shipped library on the old shape): a 17–19 sketch screen is never discovered by a 20 host, and a 20 screen never binds a 17–19 host. The two method floors born under the old shape, `MIN_API_VERSION_FOR_SKETCH_PAGES` (18) and `MIN_API_VERSION_FOR_SKETCH_TOOLS` (19), stay where they were minted — both are now **below** the action floor and are inert history, never a live gate. No other point's floor moves; only `:ext-sketch` redeclares, at **20** — it declares 20 since G3 (2026-09-18); between G2 and G3 the manifest still said 19, a build that was never installed. Not an eleventh point |
...
5. **The caller's `onDestroy` calls `finish()` too**, as the backstop for a caller destroyed while
   ...
   comes.
6. **Since arc 32 / RS1, the entry maintains the host's surface stack from its own lifecycle**:
   it pushes its `stackEntry` the instant `launcher.launch` has actually launched — never at the
...
### The bundle-version tail (arc 28 / H6)

`ExporterInfo` grew a **second** compatible parcel tail, right behind `sourceKind`:
`bundleVersion: Int = 1`, read the same way (`dataAvail()` — an old-shape descriptor simply runs
out and means 1) and for the same reason — a new capability of the `SOURCE_PAGES` container had to
become part of the contract without moving `API_VERSION`. It says the highest `PageBundle` version
that exporter's `export()` actually reads; `:ext-pdf`'s `PdfDescriptor` is the only descriptor that
declares anything but the default, `bundleVersion = PageBundle.VERSION` (2). What 2 means is
[`docs/objects.md`](objects.md)'s to define and [`docs/export.md`](export.md)'s to detail: the
container carries an endnote page per live sticky note plus a link trailer tying each one back to
its source page — `docs/extensions.md` only needs to state the seam's own rule, which is `sourceKind`'s
rule again, one field over: **the host renders at the exporter's declared ceiling, never past
it.** Facing a v1-only exporter, `ExportRender` writes the v1 shape regardless of what the notebook
holds — the notes go out as icons with no links to follow them — and the Export screen names that
in one line rather than letting the omission pass silently. No `API_VERSION` bump: the tail is
read exactly like `sourceKind`'s, so a host built before this phase simply never reads a fourth
field and keeps treating every descriptor as it always did, and an exporter built before this
...
### `advanceOutgoing` (arc 35 / HA1)

The seventh `ICalendar` method, appended after `outgoingTarget` under `API_VERSION` **10**: after
the host has drained `takeOutgoing` to its empty bundle and read `outgoingTarget` for the page it
holds, `advanceOutgoing()` moves the calendar's **next parked page** into place (both then answer
for it) and returns true, false when nothing further is parked — the ordinary answer, and always
the answer after a selection send. A Day's whole-page Send is what parks two (AM then PM). The host
asks only a calendar declaring `MIN_API_VERSION_FOR_CALENDAR_DAY_SEND` (10), on the held bind,
before `end()`, bounded at three further pages; every drained page then goes through the HV5 paper
read and lands through `NotebookActivity.receiveCalendarPages` as one undo step. See
[`docs/calendar.md`](calendar.md) § Both transfers.
...
folders, files and bytes, never a provider's own terms. Served by the one extension on the point,
**`NSE · Cloud Storage`** (`:ext-cloud`) — a second provider is baked in **there**, beside Google
Drive, never as a second extension and never as a ninth point (`docs/cloud.md` decision 15). The only
...
constructors and `CloudService`'s own call sites, and `CloudArgs` host-side **before any bind** — a
...
ends (`CloudService`, `CloudClient`, `CloudConnectClient`, `CloudConnectEntry`) carries booleans,
...
X1 and their own X-phase — where `:ext-cloud`'s door was live but skipped: `CloudService` declares 8
...
durations only) — **until arc 40's `passageText`, the one exception**, § below. The bundled `assets/bible/bsb.bible` is copied once to the extension's own
...
### Arc 38's two tails: `resolve` and `beginAt`

> The feature side — the parser, the wire grammar, the passage view, the Recents' second table —
> is `extensions/bible/docs/bible.md` § "Bible references". What follows is the seam only.

Arc 38 "Reference" (2026-09-13, a fresh user decision) appended two methods to `IBible` after
`end()` — the calendar's `render` precedent for *how* to grow a point without opening a new one:
new transaction codes (3, 4), gated by a **method** floor rather than the point's own action floor.
`ExtensionContract.API_VERSION` moved 11 → 12 to make the new floor expressible;
`MIN_API_VERSION_FOR_BIBLE_REFERENCE` = 12 gates `resolve`/`beginAt` and the notebook's two
reference doors, while `MIN_API_VERSION_FOR_BIBLE` — the floor that gates `ACTION_BIBLE` existing
at all — stays 11 untouched: a reader that only declares 11 keeps serving the plain door
unaffected, exactly as an 8-declaring cloud extension kept working after 9 and 10 landed.

- **`resolve(text): ResolvedReference?`** is the tag manager's **second call shape** again —
  bind-per-call, no store, no showing — because parsing and checking a reference needs no state:
  `BibleClient.resolve` binds, calls, unbinds, all inside `BibleEntry.resolve`'s one suspend
  function, gated on `supportsReferences` before the bind is even attempted.
- **`beginAt(store, reference)`** is `begin`'s second opening, not a new bracket: the same
  store-lending, the same `end()` closes it, with a resolved reference's wire riding alongside the
  store so the screen opens already knowing which verses to show.

Both ride the pre-existing held-bind / bind-per-call split the tag manager and the cloud point
already established — arc 38 introduced no new call shape, only a point that now uses both of the
family's two.

### Arc 40's tail: `passageText`, the one time scripture crosses

Arc 40 "Verses" (2026-09-13) appended one more method after `takeOutgoingReference` — `passageText
(wire): PassageText?` — under the same recipe (method floor, `API_VERSION` 14 → 15,
`MIN_API_VERSION_FOR_BIBLE_TEXT` = 15, `MIN_API_VERSION_FOR_BIBLE` untouched at 11). It is the first
and only Bible-seam call to answer with the words of scripture rather than a reference to them;
the feature side — the shape of what it builds, the three doors that ask for it, the page
placement and the two refusal alerts — is `extensions/bible/docs/bible.md` § "Verses on the page".
Audit row 60 below is this method's boundary check.

### Arc 42's tails: the notes index

> The feature side — the store shape, `NotesModel`, `NotesPanel`, the judgment calls and the
> Nomad walk — is `extensions/bible/docs/bible.md` § "Notes — the personal commentary". What
> follows is the seam only.

Arc 42 "Notes" (2026-09-14) appended **seven** methods to `IBible` after `passageText` (transaction
codes 7–13), `ExtensionContract.API_VERSION` 15 → 16, `MIN_API_VERSION_FOR_BIBLE_NOTES` = 16 —
`MIN_API_VERSION_FOR_BIBLE` untouched at 11, the recipe every prior Bible tail has used. Where it
differs from every earlier tail is direction: this is the seam's first **write** into the
extension's own store from outside a showing, and the host is the one driving it.

- **Six are store-taking, bind-per-call** — `replacePageNotes` / `replaceNotebookNotes` /
  `renameNotebookNotes` / `deleteNotebookNotes` / `pruneNotes` / `noteDocumentReference` — the tag
  manager's `assign` shape exactly: `BibleClient.withNotes` leases the store on IO first (the
  pre-open rule), binds for this one call, runs the method with the store riding, and revokes the
  store in `finally` on every path. Nothing is held between calls.
- **The seventh, `takeOutgoingNote()`, rides the held bind** — `takeOutgoingReference`'s twin,
  called once a showing has returned `RESULT_BIBLE_OPEN_NOTE`, **after** the bind is finished
  (`BibleEntry.onResult`): the reader has already closed by the time the host acts on what it
  handed over, the same order every other "the reader closed for this" road in the family takes.
- **`BibleClient.withNotes`** is the one new call shape this arc adds to the client, not to the
  interface: a generic `<T>` wrapper other Bible pushes are written against, so a ninth push
  method later needs no new plumbing, only a new one-line call through it.
- **The pre-open rule holds**: the store lease happens before the bind, exactly as every other
  Bible showing's does, because a cold KDF is seconds on the Nomad and must never sit inside a
  call's timeout budget.
- **`NOTES_TIMEOUT_MS` = 4 s** (`BibleClient.NOTES_TIMEOUT_MS`) — the store is already leased
  outside it, so the budget covers one bind and one small transaction only; a timeout undoes
  nothing on either side (the next push of the same page replaces it whole, and the Rebuild door
  covers everything else), so a lost push is a log line, never a dialog.
- **Two parcelables, both unmarshal-validated**: `BibleNote(noteId, pageId, pageNumber, wire, at)`
  — one Bible link's whole worth, going **in** — and `BibleNoteTarget(notebookId, pageId, kind)` —
  where a tapped Notes row points, coming **out**. Every constructor `require` is the host's whole
  check on the way in and the reader's whole check on the way out; a value that fails one never
  crosses.
- **`EXTRA_BIBLE_NOTES_ENABLED` is the showing's second boolean** (after
  `EXTRA_BIBLE_SEND_ENABLED`), set by `BibleClient.open` only when the caller has a notebook page
  it can open (the library's and the notebook's doors) and only against a reader declaring 16. The
  editor's Lookup trampoline (arc 39, row 59) never sets it: the trampoline is a live host screen
  the stopped notebook cannot see a result through, and it has no page of its own to open a Notes
  row against — a document row's Lookup push still happens (`noteDocumentReference`, fire-and-forget
  off the Binder thread), it simply never puts a Notes button on that particular showing.

Audit rows 61–63 below are these seven methods' boundary check.
...
## The Sketch point (arc 43)

> The raster sketch **as a feature** — the pencil and rubbing eraser, the data model, the screen,
> saves, undo, export, the failure table, the Nomad numbers — is `extensions/sketch/docs/sketch.md`.
> What follows is the **seam**.

### The held bind, store-less

`ISketch` is `begin(host)` / `end()` and nothing else — even thinner than the Bible point's
`begin(store)`: there is nothing to persist on the extension's own side at all (no store, arc 43
decision 2's "nothing to persist: the bookmark is the notebook's `refId`, undo is in-session, one
tool"), so the seam's one argument runs the other way — an `ISketchHost` binder the **host** mints,
the scratch pad's / document editor's shape turned store-less. `SketchClient` is
`DocumentEditorClient`'s bracket minus the store lease: mint the host binder → `ExtensionBinder.hold`
→ `begin(host)` ≤ 2 s → launch for a result; `finish()` = `end()` ≤ 15 s (the document editor's
budget, because a save flush can be waiting behind it) then unbind + revoke in `finally`.

### `ISketchHost` — the second host-side stub

`IDocumentHost`'s recipe repeated: minted per showing, every method gated first by
`SketchHostBinder`'s uid check, revoked with the unbind. Six methods at birth (API 17) —
`current()`, `requestPage(direction)`, `readSketchChunk(layer, i)`,
`saveSketchChunk(pageKey, layer, i, bytes, last)`, `requestInk(pageKey): chunkCount`,
`readInkChunk(i)` — and five more at API 18 (K5b, § below), two more at API 19 (T2). Since arc 45
"Ink" / G2 a sketch is **two rasters, one picture** (§ below): `readSketchChunk` /
`saveSketchChunk` take a raster **layer** (`SketchContract.LAYER_GRAPHITE` / `LAYER_INK`) and each
image crosses **chunked both ways** at `SKETCH_CHUNK_BYTES` = 512 KiB (`ByteChunks`, the
`TextChunks` recipe ported to bytes) independently of the other layer; `requestInk` answers **0**
for a page with no bare ink, never a refusal — the pad's Send refuses because a selection can be
made smaller, but a face has no selection to shrink, so a dense page arrives **cut** at the
transfer caps instead. A save is accepted for **any live page** of the open notebook, identity by
key, no mode routing; a key naming a deleted page is `IllegalArgumentException("Unknown page")`.
Nothing rides the screen's Intent but `EXTRA_CHROME_HIDDEN` — the editor's and the cloud connect
screen's precedent again. `RESULT_SKETCH_SHOW_PAGES` is a plain result code, not a stub method;
Back is `RESULT_CANCELED`.

### Arc 45: two rasters

> The feature side — the pencil/rubber routing by raster, the flatten, the g-paper engine work —
> is `extensions/sketch/docs/sketch.md` and `INK_PLAN.md`. What follows is the seam only.

A sketch is no longer one PNG row: it is a **graphite** raster (`SketchContract.LAYER_GRAPHITE` =
0 — the pencil's, the rubber's) and an **ink** raster (`LAYER_INK` = 1 — the gel pen's and "Bring
in ink"'s, **never erased**, the user's decision 1), each its own page-sized **lossless WebP with
alpha** (RGBA, colour-ready), its own row, its own read window and its own save accumulator.
`SketchContract.LAYERS` names the two layers in the order the host announces and flattens them
(graphite first, ink darkened over it); a layer outside that list is
`IllegalArgumentException` on either call. `SketchPageState`'s wire form grew a second byte/chunk
pair (`graphiteBytes`/`graphiteChunks`/`inkBytes`/`inkChunks` in place of one `sketchBytes`/
`sketchChunks` pair), so every state-answering call (`current`, `requestPage`, and K5b's four
structural calls) parks **both** images in the host's per-layer read windows atomically with the
state it returns; `hasSketch` / `hasLayer` answer "either raster live", not "the one row exists."
The image guard is `ImageHeader` (WebP), not `PngHeader`: a save's last chunk is checked against
it before any decode, and a mismatch or malformed header is `SKETCH_BAD_IMAGE` (renamed from
`SKETCH_BAD_PNG`, both sides rebuilt, no legacy). Because codes 3–4 changed shape **in place**
rather than growing a tail, this is not a compatible bump: `SketchContract.MIN_API_VERSION_FOR_SKETCH`
moves from its birth 17 to **20**, so a screen or host on either side of the break simply never
binds the other (§ the `API_VERSION` ledger's 20 entry).

G3 (2026-09-18) landed the face on this shape: the manifest declares **20**, `SketchLayers` is the
one place `SketchContract.LAYER_*` and `RasterLayer` translate into each other, each raster saves
under its own `SketchSaveGovernor` behind one FIFO push lock (a page's two rasters go over the
wire one after the other, never interleaved), `PendingPngPark` is renamed `PendingImagePark` (one
park slot per raster), and the encode is `WEBP_EFFORT` = 100 — the top of the dial, chosen from
Sonnet's Nomad encode table because it produced the smallest files on both rasters and was still
no slower than the PNG it replaced, run on an IO thread three seconds behind the last mark. On the
walk's lattice notebook, the `.soil` held exactly `sketch_graphite` at 146 498 B and `sketch_ink`
at 88 642 B, both a `RIFF`/`WEBP`/`VP8X` image. G4 was skipped (the walk found nothing to adjust);
frozen at G5, 2026-09-18.

### The one written-down hard refusal

`SketchContract.MAX_BYTES` = 6 MiB is a **hard refusal**, **per raster row** since arc 45 / G2 —
the SQLCipher cursor window means an image written above it can never be read back, so
`SketchRepository.save` throws `SKETCH_TOO_LARGE` for that layer before a byte is written,
verbatim through the seam, and the other layer's row is untouched. Every other row type in this
family holds "never refuse" as a design law; this is the one deliberate, written-down exception.
`WATCH_BYTES` = 4 MB is log-only, well below it, also per raster.

### K5b's five tails: page insert/delete/undo/redo from the face

> The feature side — the swipe/long-press doors, the content-aware delete warning, the face's own
> undo/redo gestures, the notebook-stack mirroring — is `extensions/sketch/docs/sketch.md`. What
> follows is the seam only.

Five methods appended to `ISketchHost` after `readInkChunk` under the method floor
`MIN_API_VERSION_FOR_SKETCH_PAGES` = 18 (`API_VERSION` 17 → 18, `MIN_API_VERSION_FOR_SKETCH`
untouched at 17 — the recipe every method-floored Bible tail already established, now on the
host-side stub instead of the extension-side one): `insertPage(direction): SketchPageState` (the
host inserts, moves the target, loads the window), `deletePage(pageKey): SketchPageState` (the
host soft-deletes the page and its descendants the notebook's own way, as one undoable action on
the **notebook's** stack), `pageContent(pageKey): Int` (bit 1 = bare strokes/objects, bit 2 = a
document — the face's delete-warning wording), `undoPage(token)` / `redoPage(token)` (a **token**
is the host's opaque name for one structural edit, riding as a compatible tail field on
`SketchPageState`, `structuralToken`, empty for every answer that is not an insert or a delete).
The two histories — the face's `SketchEdit.Structural` entries and the notebook's own undo stack —
are kept as **one** history by construction: a structural entry holds only the token, zero bytes,
so the face's byte-budgeted undo stack can never evict it, and each replay runs the notebook's own
`Action.Page` arm verbatim before moving that entry across the notebook's stack. This is the family's
first structural-undo mirroring across a screen-owning point's seam; no other point needed one
because no other screen-owning extension can insert or delete a page from outside the notebook.

### T2's two tails: the remembered tools (arc 44 "Pencils")

> The feature side — the palette, the pencil bar, the Pencil button's shade report — is
> `extensions/sketch/docs/sketch.md`. What follows is the seam only.

Two more methods appended to `ISketchHost` after `redoPage`, `API_VERSION` 18 → **19**
(`SketchContract.MIN_API_VERSION_FOR_SKETCH_TOOLS` = 19, the method-floor recipe K5b already
established, now for the host's answer to "what is this device's pencil set to?"): `toolSettings():
SketchToolSettings?` and `putToolSettings(in SketchToolSettings)`. **API 18 → 19, not a new module
floor**: `MIN_API_VERSION_FOR_SKETCH` stays 17 and `MIN_API_VERSION_FOR_SKETCH_PAGES` stays 18
untouched; a screen declaring 18 still binds, draws and turns pages, it simply does not remember
its tools. Only `:ext-sketch` redeclares.

`SketchToolSettings` (three `Int`s — `tool`/`shade`/`size`) is **indices, never values**: the host
stores three small integers and never learns what they name, so a shade's grey or a size's px lives
in `:ext-sketch`'s `SketchPalette` alone, and retuning either one strands nothing on the host side.
The constructor bounds each field to `0..SketchContract.MAX_TOOL_SETTING_INDEX` (255) — a **sanity**
bound on an unmarshalled integer, not the palette's own size, so a stored value from a build that
offered more shades or sizes than this one is still a legal parcel and reads as this build's
default. `toolSettings()` answering **null** is not a failure — it is the seam's word for "nothing
has been remembered on this device yet" (a first showing, cleared app data), and the defaults live
in `:ext-sketch`'s `SketchToolState` rather than in the host precisely so that answer can be given
honestly. The host keeps the value in its own device-local prefs
(`data/prefs/SketchToolPrefs`, `SharedPreferences("sn_sketch_tools")`) — one setting for every
notebook, never in the `.soil`, never in a backup, because an extension writes nothing to disk
itself and this is not about a page at all. Three small integers are not content, so they may be
logged.

### What K5/K5b proved on the Nomad

Discovery answered `1 provider(s) of 1 candidate(s)` at both API 17 and 18; cold open landed
straight on the face for a Sketch notebook (no reader-shaped intermediate screen); a shell
`am start` of the screen was refused and, once, exposed a `lateinit` crash in the refused
instance's own teardown (`onScreenDestroyed` before `saver` was ever assigned) that took the
extension process down **with the legitimate face still in it** — fixed with an
`isInitialized` guard, the family's second refused-`am-start`-teardown finding after the tag
manager's. `am kill` of the host behind a live face: the pending save's `DeadObjectException`
parked it, and the park re-pushed and committed on the very next `end()` once the host relaunched
through Bootstrap — no in-place reconnect was exercised because the kill took the host's process,
not just its Activity. PSS host 65.8 MB / extension 64.2 MB, well under Paintsprout Onyx's 255 MB
(the extension-store discipline and the chunked seam, not a smaller feature).
...
refusals, the path/name/id bounds, and the three consumers' own doors. Row 43 is arc 28's
`bundleVersion` tail, walked against the code at the arc-28 freeze (2026-09-06) — it opened no new
point and grew the exporter point's own contract by exactly one more field, so it is one row rather
than a run, the same shape as row 34. Row 49 is arc 32 "Resume" (2026-09-09) — a cold-launch
reopen of an extension screen, walked against the code at the RS2 freeze — it crosses no new seam
and grows no contract, so it is one row rather than a run, the same shape as rows 34 and 43. Rows
55–56 are arc 38 "Reference" (2026-09-13), walked against the code at the R3 freeze — the two
method-floored tails appended to `IBible`; rows 52–54 (the Bible point's held bind, its one store
table, its one disk exception) hold unchanged and are not repeated.
...
| 55 | **`resolve` crosses the user's own words one way and a checked, capped reply the other — bind-per-call, no showing, and neither side is ever logged (arc 38 / R1).** The Binder call's one argument is `text: String?` (the notebook's dialog field, untrimmed at the call site, unbounded on the wire itself — the extension is free to refuse anything it cannot parse); the reply is `ResolvedReference(wire, label)` or `null`, and `ResolvedReference`'s constructor is the host's **entire** check of what came back: `wire` ASCII, no `\|`, no whitespace, ≤ `MAX_WIRE_CHARS` (512); `label` non-blank, ≤ `MAX_LABEL_CHARS` (1000) — a reply that fails either `require` never unmarshals, the family's unmarshal-is-validation rule once more. `BibleService.resolve`'s own `Slog.d` logs a passage count and a duration; `BibleClient.resolve`'s does the same host-side. Neither ever logs the text or the wire. | `IBible.resolve`, `ResolvedReference` (constructor `require`s, JVM-tested via `isWire`), `BibleService.resolve`, `BibleClient.resolve`, `BibleEntry.resolve` |
| 56 | **`beginAt`'s only addition over `begin` is the wire itself, opaque to the host on the way in exactly as `resolve`'s reply is opaque on the way out — and nothing about a showing already covered by row 52 changes.** `beginAt(store, reference)` takes the same uid-bound store binder `begin` always did, plus one `String` the host never parses: `BibleService.beginAt` itself calls `ReferenceCodec.decode(reference) != null` as its own `require`, so a wire this build cannot read refuses the call rather than opening on a broken screen. The screen's Intent is still the action + `setPackage` and not one extra — the reference travels on the bind, never the Intent, `beginAt`'s whole reason for existing as a second method rather than a screen extra. | `IBible.beginAt`, `BibleService.beginAt`, `BibleSession.reference`, `BibleClient.open(reference)`, `BibleEntry.open(reference)` |
| 57 | **B9 "Send": the Bible Intent carries one boolean, and the reference comes back over the held bind, once.** `EXTRA_BIBLE_SEND_ENABLED` is set by `BibleClient.open` only for the notebook's door and only against a reader declaring `MIN_API_VERSION_FOR_BIBLE_SEND` — the calendar's `EXTRA_CALENDAR_SEND_ENABLED` shape: a boolean, no content, no id, no path (row 52's "not one extra" is amended to "one boolean"). The reader's Send parks a `ResolvedReference` in `BibleSession.outgoing` and returns `RESULT_BIBLE_SEND` with nothing on the result Intent; `BibleEntry.onResult` calls `takeOutgoingReference()` on the bind it still holds before `end()`, and the reply unmarshals through `ResolvedReference`'s constructor `require`s (row 55's check) — a reply that fails them never lands. Once-only on the reader's side; `end()` clears whatever was not taken. Never logged on either side. | `ExtensionContract.EXTRA_BIBLE_SEND_ENABLED` / `RESULT_BIBLE_SEND` / `MIN_API_VERSION_FOR_BIBLE_SEND`, `IBible.takeOutgoingReference`, `BibleSession.outgoing`, `BibleService.takeOutgoingReference`, `BibleClient.open`/`.takeOutgoingReference`, `BibleEntry.onResult`, `BibleRefFlow.insertResolved` |
| 58 | **Arc 39 "Lookup": the editor's Intent carries one boolean, its selection crosses one way as prepared words, and the answer is a code.** `EXTRA_DOCUMENT_BIBLE_AVAILABLE` is set by `DocumentEditorClient.open` from the notebook's own `BibleEntry.supportsReferences` (the host's discovery — an extension never queries for another) and only against an editor declaring `MIN_API_VERSION_FOR_DOCUMENT_LOOKUP`; the calendar's `EXTRA_CALENDAR_SCRATCH_PAD_AVAILABLE` shape — a boolean, no content, no id, no path (row 52's "nothing rides the editor's Intent" is amended to "one boolean, still no content"). `openReference(text)` is the twelfth method on the host-side stub: `gate()` first like the other eleven, the text refused blank or over `MAX_REFERENCE_CHARS` (512, the wire cap's twin — the editor's `LookupText.prepare` refuses the same before calling), the hook funnelled through `hook {}` so only the marshalable set leaves, and the reply an `int` code — `REFERENCE_OPENED` / `UNKNOWN` / `UNAVAILABLE` — never the wire, never the label: the resolved reference stays in the host (`LookupHandoff`). Neither side logs a character of the words; lengths, codes and durations only. | `DocumentContract.EXTRA_DOCUMENT_BIBLE_AVAILABLE`/`MAX_REFERENCE_CHARS`/`REFERENCE_*`, `DocumentEditorClient.open`, `DocumentHostBinder.openReference`, `DocumentHostHooks.openReference` (the `alive` gate), `NotebookActivity.lookupBibleReference`, `LookupText`, `LookupAction` |
| 60 | **Arc 40 "Verses": `passageText(wire)` takes the wire the host already stores and answers a status, never scripture on request — the constructor is the whole check, and the result crosses as a code with nothing further on any Intent.** `passageText` takes exactly the opaque wire `resolve`/`beginAt` already produce (row 55's checks, unchanged) and answers `PassageText(status, text)`, whose constructor `require`s are the host's entire validation: `STATUS_OK` requires non-blank text ≤ `MAX_TEXT_CHARS` (4000), `STATUS_TOO_LONG` requires blank text — a reply that fails either shape never unmarshals, so there is no host-side re-check to forget. The reader's Send chooser (passage mode only) returns a plain result CODE (`RESULT_BIBLE_SEND_TEXT`) with nothing riding the result Intent, exactly as `RESULT_BIBLE_SEND` always has; the host drains the take-then-text pair on the **held** bind before `end()` (`BibleEntry.onSentText`), the same order B9's plain Send already established. The notebook's two creating-door twins (the dialog's pill, the lasso's Verses on a placed reference) are **bind-per-call**, `BibleClient.passageText`'s own open/call/close, the reference-resolve shape rather than the showing's. No new extra on any screen's Intent, in either direction. Never logged on either side — status, a character count and a duration only, never a character of the text itself. | `ExtensionContract.MIN_API_VERSION_FOR_BIBLE_TEXT`, `IBible.passageText`, `PassageText` (constructor `require`s, JVM-tested), `BibleService.passageText`, `BibleClient.passageText`, `BibleEntry.passageText`/`.onSentText`, `BibleActivity.sendToNotebook`/`.leaveWith` |
| 59 | **Arc 39: the reader is opened by a live host screen, not by the stopped notebook — and that screen is exported, package-pinned and once-only.** The notebook behind the editor cannot see a child's result until the editor closes (measured on the Nomad: a first design that launched from the notebook left the reader's bind held and the second lookup refused "already showing"), so `openReference` only parks the resolved wire in-process (`LookupHandoff.park(reader, wire, callerPackage)` — the editor package the notebook holds a bind to) and the editor starts `BibleLookupActivity` **for a result** with `setPackage(<the host>)` and no extras. That screen admits only a caller whose `callingPackage` matches the park (the reader's own `HostCallerCheck` mirrored on the host's side), only while the index is open, and only within `LookupHandoff.TTL_MS`; the park is cleared on every `take`, matched or not. It then runs the ordinary showing bracket (`BibleClient.open(wire, sendEnabled = false)` — lease, hold, `beginAt`) and launches the reader with its own launcher, so the reader's `callingPackage` is the host as always; the result reaches it the moment the reader closes and the bind is finished there. `RESULT_LOOKUP_FAILED` is the one result code it adds (the editor explains); it is never on the surface stack; the "Opening…" box is its own (the stopped notebook's `OpeningOverlay` would never draw). No Send: the editor, not a page, is behind the reader. | `DocumentContract.ACTION_DOCUMENT_LOOKUP_SCREEN`/`RESULT_LOOKUP_FAILED`, `LookupHandoff`, `BibleLookupActivity`, the `<activity>` in the host manifest, `LookupAction.startLookupScreen`/`onLookupScreenReturned` |
| 61 | **Arc 42: outward on a push is ids, a name capped at 200 characters, ordinals and opaque wires — never a path, never a key, never a character of scripture.** Every one of the six store-taking pushes (`replacePageNotes`/`replaceNotebookNotes`/`renameNotebookNotes`/`deleteNotebookNotes`/`pruneNotes`/`noteDocumentReference`) carries only: canonical UUIDs (`notebookId`, `pageId`), a notebook name truncated host-side to `BibleNoteIndex.capName`'s cap **before** it ever reaches the Binder call, 1-based page ordinals, `List<String>` of live page ids (structural pushes only) and the same opaque wire every other Bible seam call has always carried. **The host never parses a wire it pushes** — `NoteRows` is the reader's own decode, and the host's part ends at `LinkPayload.referenceOf`, which only tells it a payload *is* a Bible reference, never what it says. Nothing here is a path, a raw key, or a passphrase. | `BibleNote` / `BibleNoteTarget` (constructor `require`s), `BibleNoteIndex.pushPage`/`.pushNotebook`/`.rename`/`.delete`/`.prune`/`.noteDocument`/`.capName`, `NoteRows.inserts`/`.documentInserts` |
| 62 | **Arc 42: inward, over the held bind, is one `BibleNoteTarget` of two ids and a kind — unmarshal-validated, and acted on only after the bind is finished.** `takeOutgoingNote()`'s whole reply is `BibleNoteTarget(notebookId, pageId, kind)`; the constructor is the host's entire check (both ids canonical or `pageId` empty, `kind` one of the two declared constants, a `LINK` kind never page-less) — a reply that fails it never unmarshals, so `BibleEntry.onResult` never sees a target it has to re-validate. The take happens on the bind `BibleEntry` still holds (`open`), but the target is handed to `onOpenNote` only **after** `open.finish()` has run — the reader has already closed by then, the same "closed for what it hands over" rule the Send tails established, now applied to a read that opens *another* screen instead of landing an object on the current page. | `IBible.takeOutgoingNote`, `BibleNoteTarget` (constructor `require`s), `BibleClient.takeOutgoingNote`, `BibleEntry.onResult` (`noteTarget`/`onOpenNote`), `BibleActivity.leaveWithNote` |
| 63 | **Arc 42: the store a push rides is the same uid-bound, revoked-in-`finally` binder every store-taking call on this seam gets, and one push is one transaction.** `BibleClient.withNotes` leases the store on IO first (the pre-open rule, unchanged), mints the bind for this one call, and revokes the store in `finally` regardless of outcome — nothing new about the store's own trust boundary, only a new caller of it. Each of the six methods runs its own `exec` as a single all-or-nothing transaction on the extension's side (`BibleStore`), so a partial page or a partial rebuild step is never left half-applied; a failed or timed-out call is a log line (`BibleNoteIndex`'s callers never throw to their own caller), because the index is a cache the next live push or the Rebuild door repairs, never the user's own writing. | `BibleClient.withNotes`/`.NOTES_TIMEOUT_MS`, `ExtensionStores.lease`, `BibleStore` (the seven methods), `BibleNoteIndex` (every effect function) |
| 64 | **Arc 43: `ISketch.begin(host)` runs the seam's held bind backwards — the host mints and lends the binder, not the other way round, and the extension is given nothing else to open on.** `SketchClient` mints an `ISketchHost` over a fresh `SketchHostSession` and passes it as `begin`'s one argument; the extension holds no store of its own (decision 2: "nothing to persist"), so there is no `ExtensionStoreBinder` on this point at all — the first screen-owning point with none. `SketchService.begin` calls `HostCallerCheck.enforce` first as every stub does, then parks the host binder for the session; nothing rides the screen's Intent but `EXTRA_CHROME_HIDDEN`. | `ISketch.aidl`, `SketchClient.open/finish`, `SketchService.begin/end`, `SketchSession` |
| 65 | **Arc 43, reshaped arc 45 / G2: `ISketchHost`'s chunk calls move a page's two rasters and its bare-ink strokes across, chunked both ways, and nothing else.** `current()`/`requestPage(direction)` answer a `SketchPageState` (key, index, count, width, height, a byte/chunk count **pair per raster** — validated in its own `init`); `readSketchChunk(layer, i)`/`saveSketchChunk(pageKey, layer, i, bytes, last)` move the WebP the **layer** names at `SKETCH_CHUNK_BYTES` (512 KiB) with the sender's key checked against the receiver's window on every chunk, each layer's stream independent of the other's; `requestInk(pageKey)` answers a chunk count with **0 meaning no bare ink, never a refusal** (link-wrapped and sticky ink are excluded structurally, not by a filter here) and `readInkChunk` streams the same `WireStroke` shape the pad's transfer already uses. `SketchHostBinder.gate()` runs first in every call; only the point's own request/reply parcelables ever leave the `hook {}` funnel. | `ISketchHost.aidl`, `SketchPageState` (constructor checks), `ByteChunks`, `SketchHostBinder`, `SketchHostSession`, `SketchHostHooks.loadCurrent/requestPage/commit/requestInk` |
| 66 | **Arc 43, per-raster since arc 45 / G2: `MAX_BYTES` is the one written-down hard refusal in the whole family, and it is enforced host-side before a byte crosses.** `SketchContract.MAX_BYTES` = 6 MiB is the SQLCipher cursor-window ceiling, applied **per raster row** — an image stored above it could never be read back — so `SketchRepository.save` throws `SKETCH_TOO_LARGE` (the contract's exact string) for that layer before any `saveSketchChunk` write lands, and a refused **stored** row (one that shrinks after already being saved smaller, then grows past the line on a later save) is soft-deleted rather than left half-written; the other layer's row is untouched. `SKETCH_BAD_IMAGE` (`SKETCH_BAD_PNG` until G2 — renamed with the format, both sides rebuilt, no legacy) is the `ImageHeader` guard's twin refusal, thrown before decode on either side of the seam. | `SketchContract.MAX_BYTES`/`WATCH_BYTES`/`SKETCH_TOO_LARGE`/`SKETCH_BAD_IMAGE`, `SketchRepository.save`, `ImageHeader.matches` |
...
either side** (arc 25): an email, usually, kept out of every log line in `CloudService`,
...
the way (`:ext-tags` Tabler `tag` at W1, `:ext-calendar` Tabler `calendar` at Y1, the cloud's
Tabler `cloud` at V1). The reasoning that bought those — "a person finds this point by its glyph" —
did not survive contact with the real list: the **label** is what a person reads in Settings → Apps,
the glyph only says *which family this belongs to*, and eight packages wearing eight marks said that
less clearly than eight wearing one. So the puzzle is the family mark and there is no exception;
a new extension takes it without asking. `:ext-cloud` is the **only** extension carrying
`INTERNET` — the host has none, so it is the one process in the whole app that talks to the
network.)
...
| versionName | host lockstep: `0.1.0-sn` (`-dev` in debug) |
| Release APK | 6.8 MB |
...
| API version | **per service**: the editor declares **14** since arc 39 "Lookup" (2026-09-13 — `DocumentContract.MIN_API_VERSION_FOR_DOCUMENT_LOOKUP`: it calls `IDocumentHost.openReference`, transaction code 11, which a 13 host would land on nothing; **6** from arc 22 / X4 to arc 38 — `MIN_API_VERSION_FOR_STORE`; the Document button was gone from the host between X1's landing and X4's, when this was still 2), the text importer and document exporter stay at **3** (the `resultKind` tail / `SOURCE_DOCUMENT` are load-bearing for them, and neither takes a store — an older host skips those two services at discovery and still binds nothing it would misread) |
...
| Icon | the same Tabler "puzzle" glyph as every other extension, byte-identical vector — the family mark. It wore Tabler `tag` from arc 21 / W1 until the user's 2026-09-05 call put the whole family back on one glyph: an extension is found by its **name** in Settings → Apps, and a per-subject icon bought nothing the label did not already give |
...
| Icon | the same Tabler "puzzle" glyph as every other extension, byte-identical vector — the family mark. It wore Tabler `calendar` from arc 23 / Y1 until the user's 2026-09-05 call put the whole family back on one glyph: an extension is found by its **name** in Settings → Apps, and a per-subject icon bought nothing the label did not already give |
| Launcher activity | **None** — the screen `<activity>` is exported under its own action with `<category DEFAULT>` and is refused unless launched for a result by the host; the Supernote launcher shows the package anyway, the family recipe |
| versionName | host lockstep: `0.1.0-sn` (`-dev` suffixed in debug), bumped together with `:app` at arc freezes |
...
| API version | declares **10** since arc 35 / HA1 (`MIN_API_VERSION_FOR_CALENDAR_DAY_SEND`, a *method* floor on `advanceOutgoing`; 9 from arc 31 / HV4 — `MIN_API_VERSION_FOR_CALENDAR_RENDER`, a *method* floor on `render`/`outgoingTarget` — the *action* floor `MIN_API_VERSION_FOR_CALENDAR` stays 7, and a calendar that only ever declared 7 still binds for the original four methods). The point was born at 7, so unlike the pad, the tag manager and the editor, `:ext-calendar` was never live-but-skipped between a store rebuild and its own redeclaration: there was no lower number for it to have declared first |

**`:ext-cloud`** (arc 25 / V1–V5, the THIRTEENTH module — `CloudService` + `ConnectActivity`, one APK on one point)

Renamed from `:ext-drive` / `NSE · Google Drive` on 2026-09-05, the user's call: **a second cloud
provider is baked in HERE, beside Google Drive, never as a second extension.** So the module, the
package, the applicationId, the label and the point's own service (`DriveService` → `CloudService`)
are generic, and what stays provider-named is the implementation — `DriveApi`, `DriveAuth`,
`DriveHttp`, `DriveTokens`, `DriveOps`, `DriveStore`, `DriveSql`, `DriveSchema`, `DriveFailures`,
`DriveJson`, `DriveMultipart`, `DriveRest` are Google Drive's OAuth flow and REST v3 client and are
honestly named; a second provider arrives as `Dropbox*.kt` next to them, with
`CloudService.PROVIDER_NAME` and the `opsFor` wiring as the fork. Nothing on the seam moved —
`CloudStatus.providerName` still reports "Google Drive", which is what the host's rows print, and
the host discovers by intent action, never by package. **The applicationId changed**, so the old
APK is a different package: it must be uninstalled, and because the token store is the host's
`Garden/<pkg>.db` the account is re-connected once.

| | |
|---|---|
| Label | **"NSE · Cloud Storage"** (`"NSE · Cloud Storage Dev"` in debug — a build-type string override, not a suffix) |
| Package | `com.symmetricalpalmtree.notesproutsn.ext.cloud` (`.dev` in debug) |
| Icon | the same Tabler "puzzle" glyph as every other extension, byte-identical vector — the family mark. It wore Tabler `cloud` from arc 25 / V1 until the user's 2026-09-05 call put the whole family back on one glyph: an extension is found by its **name** in Settings → Apps, and a per-subject icon bought nothing the label did not already give |
...
| API version | declares **15** since arc 40 "Verses" (`MIN_API_VERSION_FOR_BIBLE_TEXT`; 13 from B9 — `MIN_API_VERSION_FOR_BIBLE_SEND`; 12 from arc 38 / R1 — `MIN_API_VERSION_FOR_BIBLE_REFERENCE`; **11** (`MIN_API_VERSION_FOR_BIBLE`) from its first phase — the point was born at 11, so like the calendar and the cloud point, `:ext-bible` was never live-but-skipped between a store rebuild and its own redeclaration: there was no lower number for it to have declared first) |
