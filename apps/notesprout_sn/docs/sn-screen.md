...
| `core/InkTones` | arc 49 / P4 — the family's **sixteen greys** (Atelier's tones, darkest first; a shade is a *level*), pure and JVM-tested; moved here from `:ext-sketch`'s `SketchPalette`, which now reads through it |
| `notebook/PaletteBar` | arc 46 (sketch) / arc 49 / P4 (every writing face) — the **shade panel**, four rows of four swatches on `AnchoredBar`, hung under the armed pen button by its re-tap; edits one level (`armedLevel` / `onPicked`) and does not know whose |
| `notebook/ShadeIcon` + `notebook/PenShadeGlyph` | the pen glyph's body filled with its shade under a solid outline (the colour rule's one opening); `PenShadeGlyph` is the per-button, frame-silent wrapper the four writing toolbars use |
