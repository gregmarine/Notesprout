...
either bound, never disabled). **Two recorded exceptions to "bottom bars are pager-only" sit on
this strip's other end** (`apps/notesprout_sn/CLAUDE.md`): `btnBible` (arc 37 / B0, decision 3)
and, immediately to its left, `btnSketch` (arc 43, decision 9) — Sketch notebooks only, `GONE`
never disabled, re-discovered on every `onResume` like every other extension door and mirrored in
the arc-36 collapsed overflow. A tap opens the sketch face (`SketchEntry`/`openIntoSketch`); the
face's own top bar carries **Show pages** back to this screen (`RESULT_SKETCH_SHOW_PAGES`), and a
plain Back seals it to the library the way a Sketch notebook that was never opened into the face
always has — see `extensions/sketch/docs/sketch.md` and [`docs/extensions.md`](extensions.md) § "The
Sketch point" for the seam) → `selectionToolbar`
...
page and goes with the rest, and comes back on undo like everything else. A page's **sketch** is
never part of the erase (the user's arc-43 call, decision 11) — Erase page is ink-only, and a
sketch beside untouched ink is left exactly as drawn.
