...
§ "Bible references". **Arc 40 "Verses"** (2026-09-13) added a fifth kind, `KIND_BIBLE_TEXT` —
`KIND_BIBLE`'s slot rules exactly, wrapping the passage's own verses instead of a reference to
them; see § "The Bible kind" below.
...
no pageId; see below)** · **`4` the verses of a passage (arc 40 — the same slot rules as `3`; see
below)**. Versioned tag, `|` separator (ids are UUIDs), empty slot for each id the
...
| `KIND_BIBLE_TEXT` (4) | `"L1\|1\|4\|<wire>\|"` | the same passage, wrapping its **verses** rather than a reference to them — `KIND_BIBLE`'s slot rules, same wire, same follow (arc 40 "Verses") |
...
**`KIND_BIBLE_TEXT` (arc 40 "Verses", 2026-09-13)** is `KIND_BIBLE` with different words inside:
the wrapped text object holds the passage's verses (a bold label line, then the numbered verses —
`extensions/bible/docs/bible.md` § "Verses on the page") rather than the user's own reference to
them. Same wire in the same slot, same `encode`/`decode` rules, same follow (into the passage
view). The one thing the kind changes is what **Edit** means: `LinkPayload.isBibleText(payload)`
tells the two kinds apart, and a `KIND_BIBLE_TEXT` link's Edit is the ordinary `TextEditDialog`
(`BibleRefFlow.editVerses`) rather than the reference dialog, because the words are scripture to
trim or annotate, not a reference to re-resolve — the payload is left untouched by that Edit.
`LinkPayload.referenceOf(payload)` answers the wire for **either** kind, so every other reader of
"is this a Bible link" (the follow, the dead-target check, the picker's Edit routing) needs no
kind-4 special case; only Edit does.
...
**Paper and og decode kinds 3 and 4 as unusable** — a dead link there, accepted at the arc-38
wizard and extended without a fresh ask at arc 40: the family format gained a fourth kind neither
of those apps knows, then a fifth built the same way, and a payload they cannot read already
degrades to `CHROME_NONE` + the dead-target dialog by the family's own `decode`-never-throws rule,
so nothing new had to be built for either direction.

`LinkNav.planFollow` answers a fourth `Follow` case, `Follow.Bible(reference)`, for **both** kinds
— `LinkPayload.referenceOf` answers the same wire whichever kind decoded it, so the plan never
needs to know which one it is — and it is **not** a place in
...
the payload in one `Action.BibleRefEdited` undo entry. **A `KIND_BIBLE_TEXT` link routes to
`BibleRefFlow.editVerses` instead** (arc 40 "Verses") — `LinkPayload.isBibleText` is the one check
that tells the two kinds apart before either Edit path is chosen — and opens the ordinary
`TextEditDialog` on the wrapped Markdown with no resolve at all: the payload never changes, so
that Edit writes only the text (`applyWords`, the shared tail `applyEdit` was split out of at
arc 40) and skips the re-resolve `KIND_BIBLE`'s Edit always makes. Because editing can change the
wrapped text's words while its measured box very often does not ("John 3:16" → "John 3:17" is the
same width), the ordinary link-composite cache rule — "same padded size ⇒ same picture" — is wrong
for either path, so `LinkRenderer.invalidate(id)` drops the cached bitmap and the edit's own
reload rebuilds it; every other link mutation still relies on the cache rule unchanged. `LinkStore.
...
- **A Bible passage** (`KIND_BIBLE` or, since arc 40, `KIND_BIBLE_TEXT`): not a hop to anywhere in
  the library, so neither the trail nor the page-op lock is touched — `openBible(reference)` opens
  the reader over this same screen and returns by result, whichever kind the tap hit. Refused (no
  reader ≥ 12, or none at all) is the dead-target dialog with its own wording, never the
  notebook/page-gone text. § [The Bible kind](#the-bible-kind-arc-38) above has the payload's own
  detail; this is only the follow's branch of it.
...
`LinkPayloadTest` (round-trips, Paper-grammar fixtures, decode rejections, caps — **21 tests**:
19 since arc 38 / R3 added the `KIND_BIBLE` round-trip, the `reference`-not-`notebookId` decode
rule, the `isWire` boundary and the Paper/og dead-kind fixture; +2 at arc 40 / V1 for
`KIND_BIBLE_TEXT`'s own round-trip and `isBibleText`),
