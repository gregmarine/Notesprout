...
/**
 * Discovery + trust for SN's ten extension points. A candidate `<service>` is kept only if it is
 ...
 * point, born there in arc 37 / B0; 17 on the sketch point, born there in arc 43 / K2) — and it is signed
...
object ExtensionRegistry {
    ...
    /**
     * The one trusted sketch surface, or null (arc 43 / K4 — SN's **tenth** capability point and
     * its sixth screen-owning one). Same filter and first-wins rule as [bible]: a second installed
     * sketch extension is ignored with a `Slog.d`, because choosing between raster engines is not a
    ...
    suspend fun sketch(context: Context): ProviderRef? = withContext(Dispatchers.IO) {
        ...
        for (extra in all.drop(1)) Slog.d(TAG) { "ignoring additional sketch surface ${extra.component.flattenToShortString()}" }
        all.firstOrNull()
    }
