...
object TemplateThumbnails {
    ...
    /** **ARGB_8888 since arc 49 / P4** (RGB_565 before — see [MAX_CACHE_BYTES], recomputed with
     *  it). The card is erased to white and every draw lands on top of it, so there is no alpha in
     *  it; the depth is for the sixteen greys a page's strokes may now carry. */
     ...
     * The widest card is the Manta's (F4: 3 columns of a 1920 px band, ~628 x 837), and **since
     * arc 49 / P4 every card is ARGB_8888, ~2.1 MB** (RGB_565's ~1.05 MB until then) — so a
     * six-card page is ~12.6 MB and 16 MB holds it whole. Two things have to move together here:
     * make a card wider, or change its depth again, and this bound needs recomputing.
     */
