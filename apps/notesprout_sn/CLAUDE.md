...
its own — **withdrawn 2026-09-21 by g-paper Phase 37 → 0.1.52**: (g-paper Phase 38 — the flank's grain as the point's in grey, per-fleck paleness — built, walked and **withdrawn** the same day: the live path could not afford it; deferred in root `BACKLOG.md`); a mark settles **only** on the sketch face's **one-finger swipe down** (`PageGestures.onSwipeDown` → `settleDisplay()`) or a page load — the tool / shade / rub / undo / chrome-open settles of Phases 32–34 withdrawn with it; a page turned to is shown settled from its first frame). **Arc 47 "Side" (2026-09-19/20, same branch): the pencil's flank — g-paper Phase 36 → 0.1.51, no seam change; see `extensions/sketch/docs/sketch.md` § "Arc 47's decisions".** Seam:
