...
BUILT 2026-09-22** (g-paper Phase 42 → 0.1.56 on `panel-ink`, mavenLocal, demo on the Nomad),
**awaiting the user's Nomad walk**; P1 next on the walk's word): the vector-ink writing faces (notebook, pad, calendar, event note, sticky editor)
...
in § 3. Fable writes it all, no subagents. **P0 is built — read the plan's ledger, then wait for the
walk; P1 (the notebook opts in, `:sn-screen` → 0.1.56) follows only on the user's word.**
