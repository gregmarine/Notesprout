...
(pad + calendar + event note set `directInk = true`); **P3 COMPLETE 2026-09-22 on the user's Nomad walk (the sticky editor sets `directInk = true`, audit clean); P4 (greys) only on the user's word**): the vector-ink writing faces (notebook, pad, calendar, event note, sticky editor)
...
in § 3. Fable writes it all, no subagents. **P0–P3 are complete — every writing face is on the panel; post-P3 the lasso eraser's trail is an x-stream again (g-paper Phase 43 → 0.1.58 after the first walk tightened the x pitch 16 → 10 px, `:sn-screen` pinned, awaiting the Nomad walk); read the plan's ledger; P4 (the
