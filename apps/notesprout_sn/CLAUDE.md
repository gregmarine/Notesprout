...
its own — **withdrawn 2026-09-21 by g-paper Phase 37 → 0.1.52**: the settle is asked for by a **one-finger swipe down** on the sketch face (`PageGestures.onSwipeDown` → `settleDisplay()`), never by a timer; a page turned to is shown settled from its first frame). **Arc 47 "Side" (2026-09-19/20, same branch): the pencil's flank — g-paper Phase 36 → 0.1.51, no seam change; see `extensions/sketch/docs/sketch.md` § "Arc 47's decisions".** Seam:
