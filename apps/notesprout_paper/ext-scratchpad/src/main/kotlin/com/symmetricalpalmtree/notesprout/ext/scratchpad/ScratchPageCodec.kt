...
/**
 * A scratch page's store blob ⇄ (pageWidth, pageHeight, strokes) — pure, JVM-tested (arc 6 / S0).
 *
 ...
 * ```
 * [StrokeCodec] is the `.soil`'s own stroke encoding (shared from `:paper-screen`), so a page blob is
 * that format with a header. The reader tolerates a **truncated tail** (the partial stroke is dropped)
 * and skips a stroke whose geometry blob is malformed; an unknown version / header is an
 * `IllegalArgumentException` (the caller treats the page as unreadable).
 */
