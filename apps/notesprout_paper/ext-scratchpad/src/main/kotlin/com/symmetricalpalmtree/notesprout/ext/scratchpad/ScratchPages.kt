...
import com.symmetricalpalmtree.notesprout.notebook.PageMath

/**
 * The scratch pad's page-list arithmetic — pure, JVM-tested (arc 6 / S0). The store side (keys,
...
object ScratchPages {
    ...
    /**
     * Remove [id]; the landing page is the previous one (or the first). Never below one page:
     * with a single page the list is returned unchanged (the caller empties it instead) and the
     * landing id is that page.
     */
