...
import com.symmetricalpalmtree.notesprout.data.NonDestructiveOpenHelperFactory
import net.zetetic.database.sqlcipher.SQLiteDatabase as ZeticDB
import net.zetetic.database.sqlcipher.SupportOpenHelperFactory
import java.io.File

/** What a probe of a database file found. Paper never opens [Plaintext] — it has no plaintext mode. */
 ...
 *
 * Every SQLCipher open in Paper (index or `.soil`) routes through here. Never construct a
 ...
 *    opens are create-capable; pointed at a missing path they would fabricate an empty database that
 *    masquerades as the real one. Creation has its own explicitly named entry point, used only by
 *    the new-index bootstrap and the new-notebook flow.
 *  - Key encoding: passphrase to UTF-8 bytes ([keyBytes]). Identical on every platform, so a file
 *    opens with the same passphrase on any stock SQLCipher 4 build (default KDF, default page size —
 *    never customise `kdf_iter` / `cipher_page_size`).
 */
object SoilCrypto {
    ...
    // ── Room factories (Room itself is create-capable — callers guard existence; see SoilDatabase/PaperIndex)
    ...
    /** True iff [passphrase] opens [file]. False for a missing/empty file (a create-capable open would
     *  otherwise mint an empty DB keyed to whatever was typed and "verify" against nothing). */
