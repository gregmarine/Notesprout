...
/**
 * The one bind-per-operation path every extension client uses (M1 — extracted verbatim from the
 * verified `TemplateProviderClient` / `NamerClient` bodies): explicit component, **signature
 * re-checked at bind time** (not only at discovery — the package could have been replaced with the
 * same name and a different key while the screen holding the [ProviderRef] was open),
 * `BIND_AUTO_CREATE` on the app context, connection awaited ≤ [bindTimeoutMs], [block] run on IO
 * under [callTimeoutMs] inside a supervisor scope, **unbind in `finally`**. The core never holds a
 * binding across screens.
 *
 ...
 *
 * Clients wrap this with their own payload validation (and, for the store-holding namer, the store
 * pre-open + per-bind binder mint/revoke *around* the call). Log lines go under the caller's [tag].
 *
 * [hold] (arc 6 / S0) is the bind half of [call] without the unbind — for the one case where "the
 ...
 * caller's `finally` when the screen is over. Still bind-per-operation (rule 26) — the operation is
 * just longer.
 */
object ExtensionBinder {
    ...
    /**
     * A bind that outlives one call (arc 6 / S0). [call] runs [block] on IO under [timeoutMs] with the
     * same exception mapping as [ExtensionBinder.call]; after the connection dies (`onBindingDied` /
     * `onServiceDisconnected`) or [close] every call throws [ExtensionCallException]. [close] unbinds
     * (idempotent) — the caller's `finally`.
     */
    class HeldBinding<I : Any> internal constructor(
        ...
        val iface: I,
        private val deadFlag: java.util.concurrent.atomic.AtomicReference<String?>,
    ) {
        ...
        /** Unbind; idempotent. Orphaned calls are cancelled (their Binder transaction finishes on its own thread and is discarded). */
        fun close() {
            if (closed) return
            closed = true
            scope.cancel()
     ...
     * bind, `BIND_AUTO_CREATE` on the app context, connection awaited ≤ [bindTimeoutMs]. On any
     * failure the (attempted) bind is released before the [ExtensionCallException] is thrown; on
     * success the caller owns the returned [HeldBinding] and must [HeldBinding.close] it.
     */
    suspend fun <I : Any> hold(
        ...
        val dead = java.util.concurrent.atomic.AtomicReference<String?>(null)
