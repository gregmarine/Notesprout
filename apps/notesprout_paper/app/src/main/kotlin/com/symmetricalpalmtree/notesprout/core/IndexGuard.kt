...
import com.symmetricalpalmtree.notesprout.bootstrap.BootstrapActivity
 ...
 * launcher and finishes itself once it has forwarded, so it is not on the back stack. The one route
 * into a screen that never passes through it is Android rebuilding a task's activities by itself
 * after a background process kill (tap the app in Recents on a memory-tight e-ink device). Nothing has
 * opened the index in that process, and the first read would throw.
 *
...
object IndexGuard {
    ...
    /**
     * True when the index is open. Otherwise sends the task back through [BootstrapActivity] as the
     * root of a clean task (`NEW_TASK | CLEAR_TASK`), finishes [activity], and returns false —
     * **return immediately**. Idempotent via [Activity.isFinishing].
     */
    fun ready(activity: Activity): Boolean {
        if (PaperIndex.isReady()) return true
        bounced += activity
