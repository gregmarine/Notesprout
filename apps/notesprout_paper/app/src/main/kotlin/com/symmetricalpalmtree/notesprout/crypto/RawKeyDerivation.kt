 ...
 * CPUs). The output is the same key every time, so it is derived once, cached ([KeyMaterial]), and
 * later opens pass `x'<hex>'` which SQLCipher applies directly (~35 ms). The derived key is byte-exact
 * to SQLCipher's own KDF output over the file's 16-byte plaintext salt, so the file stays
 * passphrase-portable. Pure JVM apart from `java.io.File`; key material is NEVER logged.
 */
