...
import com.symmetricalpalmtree.notesprout.data.indexFile
 ...
 *
 * **`BootstrapActivity` is the only caller of [ensureReady] / [unlockAndOpen].** Every other screen
 * checks [isReady] through `IndexGuard` in `onCreate` and bounces back to Bootstrap when the answer
 * is no (a task Android rebuilt after a background process kill lands there). **Nothing ever closes
 * it** — it opens once per process and stays open — which is what makes a single `onCreate` check
 * sufficient.
 *
 * Open state machine (probe the file header, never open to find out):
 *  - `Invalid` (no file / empty) → mint (or reuse) the global key → create encrypted → derive + cache
 *    the raw key → [PrepareOutcome.FIRST_LAUNCH] (the caller shows the recovery key once).
 ...
 *  - `Plaintext` → impossible for Paper (there is no plaintext mode). Never opened: it would either
 *    be a foreign file or a damaged one, and the framework's corruption path deletes. [PrepareOutcome.FOREIGN_FILE].
 *
 * There are no in-place migrations in v0, so no repair-before-probe swap sweep exists yet; add one at
 * the top of [ensureReady] if a later version introduces a file swap.
 */
object PaperIndex {

    enum class PrepareOutcome { READY, FIRST_LAUNCH, NEEDS_UNLOCK, FOREIGN_FILE }

    private const val TAG = "PaperIndex"
    ...
    fun db(): IndexDatabase =
        instance ?: throw IllegalStateException("PaperIndex is not open — BootstrapActivity must run first")
            ...
            when (SoilCrypto.probe(file)) {
                ...
                SoilFileKind.Encrypted -> {
                    ...
                    if (!SoilCrypto.verifyRawKey(file, key)) {
                        // The cached material no longer opens this file (restored from elsewhere, etc.).
                        // Drop the derived key; if the passphrase itself is right, unlock re-derives.
