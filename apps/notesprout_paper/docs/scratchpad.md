...
`PAPER_SCRATCHPAD_PLAN.md` (S0 ✅ · S1 ✅ · S2 ✅ · S3 ✅ — arc complete; **this file is frozen**).
...
| Back | the current page flushed, `RESULT_CANCELED`, **`releaseForHandoff()` then `finish()`** (every exit — Back, Send, the store dialog — goes through `finishWithHandoff`: the notebook reclaims the process-global EPD pipeline in its `onResume`, which runs before this window's visibility close; a close landing after that reclaim tore the notebook's live session down on BOOX — ink and lasso trails invisible until a tool flip; on Supernote the pad's *later* focus-loss / `release()` teardowns did the same to the panel's full-UI-auto mode → slow-waveform drags, because g-paper's ownership guard is per-process — g-paper 0.1.2 makes the Ratta handoff drop the token), the host finishes the bind |
