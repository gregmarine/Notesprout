...
/**
 * Pure geometry: turns a flat list of a page's [InkStroke]s into reading-order lines and
 * paragraphs. No ML, no Android; unit-testable in isolation.
 *
 * **Verbatim port of the original Notesprout `recognition/StrokeSegmenter`** (`RectF` → [Box],
 * `LiveStroke` → [InkStroke]; every constant and step unchanged — behaviour must not drift).
 *
...
object StrokeSegmenter {
    ...
    /** A stroke with its precomputed bounds (the original carried the AABB on `LiveStroke`). */
    ...
    data class Paragraph(val lines: List<Segment>) {
        val bounds: Box
            get() {
                var r = lines.first().bounds
                for (l in lines.drop(1)) r = r.union(l.bounds)
                return r
            }
    }
    ...
    fun segment(strokes: List<InkStroke>): PageLayout {
        val usable = strokes.filter { it.size >= 2 }.map { Boxed(it, Box.of(it.x, it.y)) }
            .filter { it.box.height >= 0f }
        if (usable.isEmpty()) return PageLayout(emptyList())

        val medianStrokeH = medianOf(usable.map { it.box.height }).coerceAtLeast(1f)
        ...
        val members: List<MutableList<Boxed>> = when {
        ...
        for (i in 1 until rawSegments.size) {
            ...
            if (fragment && last.bounds.verticalOverlapFrac(seg.bounds) > MERGE_OVERLAP_FRAC) {
        ...
        var cur = mutableListOf(segments.first().toSegment())
        for (i in 1 until segments.size) {
            val gap = segments[i].bounds.top - segments[i - 1].bounds.bottom
            if (gap > paraGap) {
                paragraphs += Paragraph(cur)
                cur = mutableListOf(segments[i].toSegment())
            } else cur += segments[i].toSegment()
        }
        paragraphs += Paragraph(cur)
    ...
    /** Internal line: carries [Boxed] strokes so each AABB is computed exactly once. */
    private class BoxedSegment(val strokes: List<Boxed>, val bounds: Box) {
        fun toSegment() = Segment(strokes.map { it.stroke }, bounds)
    }

    /** Build a line from strokes: order left→right and union their bounds. */
    private fun makeSegment(strokes: List<Boxed>): BoxedSegment {
        val ordered = strokes.sortedBy { it.box.left }
        var bounds = ordered.first().box
        for (x in ordered.drop(1)) bounds = bounds.union(x.box)
        return BoxedSegment(ordered, bounds)
    }
