...
### K6 — g-paper Phase 20 → 0.1.33 "A mark says where it landed" ✅ (2026-09-15)

**Landed** (g-paper Phase 20, published to mavenLocal by the user; SN re-pinned
`sn-screen/build.gradle.kts` 0.1.32 → **0.1.33**). Opus on a Fable brief, Fable read the diff
before the publish (decision 13) — one change from the review: the two new span/cap constants
are `internal`, not public API. No phase-start questions (the row lists none).
- **Core:** `RasterDirty.along(points, width, pageW, pageH, maxSpanPx = 256, maxRects = 64)` —
  pure, beside `of` and calling it once per run (one pad / snap / clip rule). The polyline is cut
  where the next point would push the run's **unpadded** span past 256 px; **the closing point
  belongs to both runs**, so the boundary segment lies wholly inside the next rect and
  consecutive rects overlap by the pad; past 64 rects the last run absorbs the tail (a bigger
  rect, never a dropped one); off-page runs are dropped; a mark that never reaches the span is
  exactly the one rect `of` always gave it (pinned). `commitCapturedStroke` and `addStrokes` in
  RASTER fire **every** `onRasterWillChange` before the composite and every `onRasterChanged`
  after, in the same order; `addStrokes` concatenates the runs of all its strokes around one
  composite. **`loadPageRaster` is silent**, as `swapPageRaster` always was — a page the host
  replaced is the host's own news. `loadStrokes` (the bake), `addStrokes` and `clear()` still
  report. `PaperListener` / `PaperView` KDoc, `docs/api.md`, `host-responsibilities.md`,
  `architecture.md`, `CLAUDE.md` bullet, `PLAN.md` Phase 20. `RasterDirtyTest` +8 → **core 215**
  (debug and release), ratta 12; `:demo:assembleDebug` green; **Paintsprout Onyx 203 green** on
  its 0.1.31 pin (its watch-list items 1 and 2 struck through in `ONYX_PLAN.md`; its own
  `loadingRaster` is dead the moment it re-pins — its decision).
- **SN:** the face's `loadingRaster` flag and its two guards are gone (`SketchActivity.loadPage`
  calls `loadPageRaster` bare); the undo-entry log line gains the main-thread milliseconds spent
  reading the contact's before-image (K6's measurement, kept — it is one `nanoTime` pair per
  will-change). `apps/notesprout_sn/CLAUDE.md` pin note. **1805 `:app` / 3538 root** JVM green.

**Measured (Nomad, the user's hand, 2026-09-15 — one corner-to-corner pencil hairline on a
fresh 1404×1685 page, then undo, then redo):**
...
4.1× fewer cells and bytes for the worst-case mark; the diagonal's cost is now the ink's
neighbourhood, not the page. Two page turns after the drawing produced **no undo entry and no
dirty page** — the load is silent and the guard that swallowed it was not missed. The user's hand
walked the undo and the redo and reported no defect ("Done").

**Trap recorded:** the ext-sketch debug APK reads 16 MB on disk for 8.7 MB of entries — the known
zipflinger inflation, not a dependency change.
