...
/**
 * Where a guide grid's lines fall on a page (arc 51 "Guides" / J3, the user's decision 3) — pure
 ...
 * dozen cells drifts by the float error of every step — the recorded `templates.md` trap — while
...
object GridLayout {
    ...
    data class Plan(val kind: Int, val xs: List<Float>, val ys: List<Float>, val cell: Float) {
        /** How many dots a [SketchContract.GRID_DOTS] grid draws — every crossing of the two lists. */
    ...
    fun plan(kind: Int, count: Int, pageWidth: Int, pageHeight: Int): Plan? {
        if (kind == SketchContract.GRID_OFF || count < 1 || pageWidth <= 0 || pageHeight <= 0) return null
