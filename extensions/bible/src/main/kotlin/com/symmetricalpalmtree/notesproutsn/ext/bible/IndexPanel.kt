 ...
 * subject. A full-window `Dialog` over the reader; **one layout, two forms** — `dialog_index.xml`
 * branches in code on [IndexLayout.fullScreen]: below 480 dp the panel fills the screen behind a
 ...
 *
 * What it shows is [IndexModel]'s and is JVM-tested; this file is only views. The cells are the
...
class IndexPanel(
    ...
    private var items: List<IndexModel.Item> = emptyList()
    ...
    fun show() {
        ...
        val root = dialog.findViewById<FrameLayout>(R.id.indexRoot)
        val panel = dialog.findViewById<LinearLayout>(R.id.indexPanel)
        body = dialog.findViewById(R.id.indexBody)
        val btnBack = dialog.findViewById<AppCompatImageButton>(R.id.btnIndexBack)
        rows = dialog.findViewById(R.id.indexRows)
        pager = dialog.findViewById(R.id.indexPager)
        pageLabel = dialog.findViewById(R.id.indexPageLabel)
        val btnFirst = dialog.findViewById<AppCompatImageButton>(R.id.btnIndexFirst)
        val btnPrev = dialog.findViewById<AppCompatImageButton>(R.id.btnIndexPrev)
        val btnNext = dialog.findViewById<AppCompatImageButton>(R.id.btnIndexNext)
        val btnLast = dialog.findViewById<AppCompatImageButton>(R.id.btnIndexLast)
        ...
        val fullScreen = IndexLayout.fullScreen(widthDp)
        if (fullScreen) {
            ...
            panel.layoutParams = FrameLayout.LayoutParams(
                IndexLayout.sidebarWidthPx(metrics.widthPixels), FrameLayout.LayoutParams.MATCH_PARENT,
            )
        ...
        btnLast.setOnClickListener { goToListPage(IndexModel.pageCount(items.size, itemsPerPage) - 1) }
        ...
        items = IndexModel.items(books, expanded)
        ...
        rows.viewTreeObserver.addOnGlobalLayoutListener(object : ViewTreeObserver.OnGlobalLayoutListener {
            override fun onGlobalLayout() {
                ...
                itemsPerPage = IndexLayout.itemsPerPage(
                    rows.height - rows.paddingTop - rows.paddingBottom, metrics.density,
                )
                val target = IndexModel.indexOfChapter(items, current.usfm, current.chapter)
                    .takeIf { it >= 0 } ?: IndexModel.indexOfBook(items, current.usfm)
                listPage = IndexModel.pageOf(target, itemsPerPage)
                render()
                Slog.d(TAG) {
    ...
    private fun goToListPage(page: Int) {
        val clamped = IndexModel.clampPage(page, IndexModel.pageCount(items.size, itemsPerPage))
    ...
    private fun toggle(book: BookRow) {
        ...
        items = IndexModel.items(books, expanded)
        val anchor = IndexModel.indexOfBook(items, book.usfm)
        listPage = IndexModel.pageOf(anchor, itemsPerPage)
        render()
    }

    private fun render() {
        rows.removeAllViews()
        val pageCount = IndexModel.pageCount(items.size, itemsPerPage)
        listPage = IndexModel.clampPage(listPage, pageCount)
        ...
        for (item in items.subList(start, end)) {
            rows.addView(
                when (item) {
                    is IndexModel.Item.Book -> bookRow(inflater, item)
                    is IndexModel.Item.Chapters -> chapterRow(item)
                },
            )
        }
    ...
    private fun bookRow(inflater: LayoutInflater, item: IndexModel.Item.Book): View {
        val row = inflater.inflate(R.layout.item_index_book, rows, false)
    ...
    private fun chapterRow(item: IndexModel.Item.Chapters): View {
        val line = LinearLayout(activity).apply {
            orientation = LinearLayout.HORIZONTAL
            gravity = Gravity.CENTER_VERTICAL
            layoutParams = LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT, IndexLayout.rowPx(activity.resources.displayMetrics.density),
            )
