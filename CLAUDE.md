...
- `extensions/sketch` — **NSE · Sketch**, the raster-sketch extension for Notesprout SN (the tenth
  ...
  levels 0 · 5 · 9 · 15, the white lead a lightener over graphite — g-paper 0.1.40; merge pending
  the user's word)
...
- `ink` — arc 45 "Ink" COMPLETE + FROZEN 2026-09-18 on the user's Nomad hand walks. It carries the
  ...
  `INK_PLAN.md` there is history (plan + ledger — read for *why*, never resumed). Also carries
  the 2026-09-18 pre-merge palette narrowing (four firmware tones incl. a white lightener lead;
  g-paper Phase 27 → 0.1.40). Merge to `main` in either repo pending the user's word. **No ELEVENTH extension point and no new arc without a
  user decision.**
