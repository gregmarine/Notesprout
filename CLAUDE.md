...
- ✂️ Pruning: Un-heading undo/redo recognizedText fix (Prompt 6) — reviewed the `removeHeading()` / `btnRemoveHeading` path end-to-end; confirmed `recognizedText` is NOT leaked into restored stroke rows (strokes are re-inserted via `StrokeData.toJson()` which has no text field, `LiveStroke` has no `recognizedText`); found that `UndoRedoAction.HeadingRemoved` was missing `recognizedText`, causing undo to reconstruct the in-memory heading without its text (heading would render as strokes and disappear from the TOC after undo); patched: `HeadingRemoved` gains `val recognizedText: String? = null`, `removeHeading()` now passes `recognizedText = heading.recognizedText` when pushing the action, and both the same-page and cross-page undo paths now pass `recognizedText = action.recognizedText` when constructing the restored `HeadingStroke`; redo path was already correct (restores stroke rows by ID, no in-memory heading reconstruction needed)

- 🌱 New Branch: Heading text edit undo/redo (Prompt 7) — `UndoRedoAction.HeadingTextEdited(headingId, pageId, previousText: String? = null, newText: String)` added; `updateHeadingText` captures `previousText = heading.recognizedText` before the coroutine and pushes the action after the DB write and in-memory update complete; `executeAction` handles `HeadingTextEdited` with both cross-page two-phase path (navigate + apply DB write + reload from DB + rebuild canvas) and same-page path (DB write in `when` block + reload from DB + rebuild canvas, preserving selection box position if the heading is still selected); audit of all heading-carrying undo actions: (1) `HeadingCreated` was missing `recognizedText` — added field with `= null` default, populated at push site, and used in both same-page and cross-page redo in-memory reconstruction; (2) `StrokesMoved` DB write was losing `recognizedText` — both same-page and cross-page DB writes now pass `recognizedText = heading.recognizedText` to `HeadingObject`; (3) `LassoCut` redo clipboard reconstruction was dropping `recognizedText` — both same-page and cross-page clipboard `HeadingStroke` copies now include `recognizedText = h.recognizedText`; `LassoErased`, `LassoDeleted`, `LassoPasted`, and `HeadingRemoved` were all correct — they fetch headings from DB via `HeadingObject.fromJson` which preserves `recognizedText`, or carry `HeadingStroke` objects that already serialize the field

Next up: TBD — discuss before starting.

---
*Last updated: 🌱 New Branch — Heading text edit undo/redo (Prompt 7)*
