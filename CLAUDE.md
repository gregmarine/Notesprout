...
- `extensions/sketch` — **NSE · Sketch**, the raster-sketch extension for Notesprout SN (the tenth
  ...
  own `INK_PLAN.md`; COMPLETE on the user's Nomad hand walk 2026-09-18; the same day, pre-merge,
  the pencil palette narrowed to the firmware's four tones — black · grey · light grey · white,
  ...
  "Palette"** 2026-09-19 on branch `tools` — **`tools` merged to `main` 2026-09-24 (`--no-ff`) and deleted; "on tools" means `main`** — (Atelier's sixteen tones for pencil and gel pen alike,
...
- `ink` — arc 45 "Ink" COMPLETE + FROZEN 2026-09-18 on the user's Nomad hand walks; **merged to
  `main` 2026-09-18 (`--no-ff`) and deleted** (local + remote) — "on ink" means `main`. It carried
  the growth of `extensions/sketch` (**NSE · Sketch**) into two rasters — graphite and ink, ink
  never erased — with the engine work in `~/git/g-paper` on branch `two-rasters` (Phase 26 →
  ...
  there is history (plan + ledger — read for *why*, never resumed).
- `tools` — **`tools` merged to `main` 2026-09-24 (`--no-ff`) and deleted** (local + remote) — "on tools" means `main`; the next sketch-tool arc opens a fresh branch on the user's word. It carried arcs 46–51: **arc 51 "Guides" COMPLETE + FROZEN 2026-09-24** on the user's Nomad walks (plan `extensions/sketch/GUIDES_PLAN.md`): a grid and a reference image under the sketch as tools — g-paper Phase 46 `setSheet` → 0.1.61 (branch `sheet` **merged to g-paper `main` 2026-09-24 and deleted — g-paper has only `main`**), rows `guide_grid` + `guide_image`, API 22; before it **arc 49 "Panel" COMPLETE + FROZEN 2026-09-22** on the user's Nomad walks (plan `apps/notesprout_sn/PANEL_INK_PLAN.md`): every vector-ink writing face — notebook, pad, calendar, event note, sticky editor — goes direct to the Supernote panel like the sketch face (g-paper Phase 42 "Ink on the panel" → 0.1.56, `PaperView.directInk`, the committed picture as the flatten base, an app-painted lasso trail), the lasso eraser's trail an x-stream (Phase 43 → 0.1.58), **sixteen greys back on every writing face's pen re-tap** (P4 — the user's reversal of R3's shade removal for the shade alone: `PaletteBar` / `ShadeIcon` / `core/InkTones` moved into `:sn-screen`, one device-wide level in the host's `PenShadePrefs`, carried to the pad and calendar as `EXTRA_PEN_SHADE` on the launch and result Intents, no API bump, exports `ARGB_8888`), and the scribble-erase hit test made near-linear after a 15 s ANR on the walk (Phase 44 `PolylineIndex` → 0.1.59); g-paper `panel-ink` (Phases 42–44) **merged to g-paper `main` 2026-09-22 (`--no-ff`) and deleted — g-paper has only `main`**; before it arc 48 "Smudge" **COMPLETE + FROZEN 2026-09-22** on the user's Nomad walks (the
  one-finger rub that blends graphite; g-paper Phase 40 → 0.1.54 on `smudge`, Phase 41 → 0.1.55
  on `always-dither` — the panel always dithered, exports true grey — **both merged to g-paper `main` 2026-09-22 and deleted, with `settle-gesture`, `ink-over`, `ink-true`; g-paper has only `main` now**;
  `extensions/sketch/SMUDGE_PLAN.md` is the plan + ledger); **arc 50 "Stump" COMPLETE + FROZEN 2026-09-23 on the user's six Nomad walks** (the smudge's smear follows the hand — g-paper Phase 45 → 0.1.60, the blend's box turned to the line of travel on the lattice's four axes, one-sided and travel-gated, `across` 4; a stylus Smudge tool `Tool.SMUDGE` at 24 px on the sketch face's top bar after the rubber, the finger rub staying; tip-switch chatter hardened; **the sketch save has a 15 s deadline** (`SketchSaveCadence`); g-paper `smudge-tool` **merged to g-paper `main` 2026-09-23 (`--no-ff`) and deleted — g-paper has only `main`**; `extensions/sketch/STUMP_PLAN.md` is the plan + ledger); before it arc 47 "Side" **COMPLETE + FROZEN 2026-09-20** on the user's Nomad and Manta walks
  ...
  **COMPLETE + FROZEN 2026-09-19** on the user's Nomad and Manta walks
  (g-paper's `ink-over` + `ink-true` merged to g-paper `main` 2026-09-22 and deleted): Atelier's sixteen tones for the sketch
  ...
  the same way (0.1.45–0.1.50, branch `ink-true`, on top of it), `API_VERSION` 20 → 21 as a parcel tail. `extensions/sketch/PALETTE_PLAN.md` is the plan +
  ledger. Every `*_PLAN.md` of arcs 46–51 is history (plan + ledger — read for *why*, never resumed). **No ELEVENTH extension point and no new arc without a user decision.**
