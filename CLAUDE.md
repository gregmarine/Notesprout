...
- `extensions/sketch` — **NSE · Sketch**, the raster-sketch extension for Notesprout SN (the tenth
  ...
  0.1.50, incl. a 2.5 s pause settle — withdrawn 2026-09-21 along with every tool / shade / rub / undo / chrome-open settle — a mark settles only on a one-finger swipe down or a page load, g-paper Phase 37 → 0.1.52; Phase 38 → 0.1.53 the same day: the flank's grain is the point's in grey — per-fleck paleness, not bare sites; its own `PALETTE_PLAN.md`)
