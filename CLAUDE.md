...
| **Tasks:** the to-do surface (`TasksActivity`) — independent of calendar and notebook. Fully columnar `tasks` table (v9, **no `data` JSON**), materialized recurrence (one open row at a time; resolving generates the successor), next-due = first occurrence after `max(due, action day)`, Done/Skip/un-complete, three entry points, routines reserved via `type`/`parentId` | [`docs/tasks.md`](docs/tasks.md) |
