...
- `extensions/sketch` — **NSE · Sketch**, the raster-sketch extension for Notesprout SN (the tenth
  ...
  "Palette"** 2026-09-19 on branch `tools` (all sixteen greys for pencil and gel pen alike, one
  4 px pencil lead, the shade picker on its own `palette` button with Atelier's swatches, both pen
  glyphs filled with their own shade, `SketchToolSettings.penShade` as a parcel tail — API 20 → 21,
  no floor moved — no g-paper change; its own `PALETTE_PLAN.md`)
...
- `ink` — arc 45 "Ink" COMPLETE + FROZEN 2026-09-18 on the user's Nomad hand walks; **merged to
  ...
  there is history (plan + ledger — read for *why*, never resumed).
- `tools` — **ACTIVE** (2026-09-19): arc 46 "Palette" — sixteen greys for the sketch face's pencil
  and gel pen, one 4 px pencil lead, the shade panel on its own Palette button, both pen glyphs
  wearing their shade, `API_VERSION` 20 → 21 as a parcel tail. Expected to carry further tool arcs
