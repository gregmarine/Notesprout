...
| Calendar: handwriting-first Month/Week/Day canvas, `calendar` table (v3) + keyed pages, template renderer (grid/timeline + hit-test), page-load contract, finger gestures (swipe/tap/double-tap-to-open/multi-finger undo-redo), last-position persistence, lasso/clipboard parity, Send to Notebook, full-view export (grid→page template + optional writing, toolbar top-margin), day-detail three-view "day window" (`DayDetailActivity`: **Note** editable canvas `cal-daynote-` + copy-into-table templates · **Notebooks** Opened/Edited/Created card grid · **History** past-year picker + read-only day-note bitmap; `notebook_activity` log v4/`MIGRATION_3_4`, `DayHistoryRepository`, back = step-out to Note) | [`docs/calendar.md`](docs/calendar.md) |
