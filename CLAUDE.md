...
| **Tasks:** the to-do surface (`TasksActivity`) — independent of calendar and notebook. Fully columnar `tasks` table (v10, **no `data` JSON**), materialized recurrence (one open row at a time; resolving generates the successor), next-due = first occurrence after `max(due, action day)`, **look-ahead reminder gates *Upcoming*** (a dated task with no reminder is not shown until it is due — the **All** view is the ungated escape hatch that keeps it reachable), Today/All/Done views (Events-style chrome; no date control — the list is always relative to the real today, kept fresh by a date-change receiver; **Done is windowed to 30 days** + "Show N earlier", since recurring tasks accrue resolved rows forever), Done/Skip/un-complete, three entry points, routines reserved via `type`/`parentId` | [`docs/tasks.md`](docs/tasks.md) |
